...
class Invoicesupplier extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('username')) {
            ...
            $data['menu'] = $res_menu;
            $res_ro = $this->receiveorder_models->receiveorder_content_invoice();
            $data['rcreceiveorder']=$res_ro;
            $this->load->view('listinvoicesupplier', $data);
        } else {
             redirect(base_url());
        }
    }

    public function daftar_produkinvoice_supplier()
	{
		if($this->uri->segment(3))
		{
				$id['id_ro'] = $this->uri->segment(3);
                $sql = "select a.no_ro,a.no_po,a.no_pr,a.kd_supplier,b.nama_supplier 
                            from mst.tt_receive_order a, mst.tm_supplier b 
                            where a.kd_supplier=b.kd_supplier and a.id_ro='".$id['id_ro']."'";
                #exit($sql);
				...
				$this->load->view('page/vw_detailinvoice_supplier',$data);
		}
		else
		{
            redirect(base_url());
