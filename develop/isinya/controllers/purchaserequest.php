...
class Purchaserequest extends CI_Controller {
    ...
    public function index() {
        ...
        $data = array(
            ...
            'title' => $judul,
            'location' => 'Home - Master - Purchase Request'
        );
        ...
        if ($this->session->userdata('username')) {
            ...
            $this->load->view('pr/vwpurchaserequest', $data);
        } else {
             redirect(base_url());
        }
    }
    ...
    public function form() {
        ...
        if ($this->session->userdata('username')) {
            $this->load->view('pr/purchaserequest', $data);
        } else {
             redirect(base_url());
        }
    }
	...
	public function daftar_Produk()
	{
		...
		if(!empty($cek))
		{
			...
			$bc['tm_produk'] = $this->app_model->manualQuery("select kd_produk,nama_produk from mst.tm_produk limit 100");
			
			$this->load->view('pr/daftar_produk',$bc);
		}
		else
		{
            redirect(base_url());
...
<!--			<tr>
			...
			<td>Qty</td>
			<td><input type="text" id="qty" name="qty" style="width:50px;" onKeyPress="return onlyNumbers(event);" class="required number" /></td>
			</tr>
			</tbody>
			</table>
			<?php
