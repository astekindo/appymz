...
class Receiveorder_models extends CI_Model {
	...
	public function receiveorder_content_invoice() {
        $user = $this->session->userdata('kodex');
        if (isset($user)) {
            $sql1 = "select *,to_char(created_date,'dd-mm-yyyy hh:mm:ss') complite_date							
						FROM mst.tt_receive_order WHERE status_invoice=0";
            $query = $this->db->query($sql1);
			$no=0;
            foreach ($query->result() as $row) {
				$no=$no+1;
                ...
                $this->receiveorder.='<td align="center">' . $no . '</td><td align="center">' . $row->no_ro . '</td>
									<td>' . $row->no_po . '</td><td align="center">' . $row->no_pr . '</td><td align="center">' . $row->kd_supplier . '</td>
									<td align="center"><a href="invoicesupplier/form/'. $row->no_ro .'" />
										<input type="button" style="float: center;margin-top:-10px;margin-right:5px;" name="create_invoice" id="create_invoice" value="Create Invoice" class="buttonM bRed" />
										</a></td>
									';
                $this->receiveorder.='</tr>';
            }
