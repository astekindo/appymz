...
class Laporan_purchase_order_model extends MY_Model {
    ...
    public function get_data_po_print(){
		...
		$sql = " select * from report.v_lap_purchase_order limit 6 ";
    ...
    public function search_member($search = "", $offset, $length){
		if($search != ""){
			$this->db->where("((lower(nmmember) LIKE '%" . $search . "%') OR (kd_member LIKE '%" . $search . "%'))", NULL);
		}
