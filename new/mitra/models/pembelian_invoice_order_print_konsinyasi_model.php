...
class pembelian_invoice_order_print_konsinyasi_model extends MY_Model {
    ...
    public function get_rows($kd_supplier="",$kd_peruntukan = "", $search = "", $offset, $length) {
        ...
        if ($search != "") {
            ...
            $sql_search = " AND (lower(a.no_invoice) LIKE '%" . strtolower($search) . "%')";
    ...
    public function get_data_print($no_invoice = '') {
        ...
        $sql_detail = "SELECT a.*,b.*,nama_produk, nm_satuan, e.tanggal
				FROM purchase.t_invoice a
				JOIN purchase.t_invoice_detail b
					ON a.no_invoice = b.no_invoice
				JOIN mst.t_produk c
