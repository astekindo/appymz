...
class Approval_manager_model extends MY_Model {
    ...
    public function get_rows($kd_peruntukan ="", $search = "", $offset, $length) {
        ...
        $this->db->select("a.waktu_top,no_ro,tgl_ro,subject,nama_supplier, case when a.status = '1' then 'YES' else 'NO' end app_ass_manager", FALSE);
    ...
    public function get_rows_detail($search = "") {
        ...
        $sql1 = "SELECT  
				...
				WHERE " . $sql_search . " (a.status='0' OR a.status='1') AND a.konsinyasi = '0'
				GROUP BY 
					b.no_ro, b.kd_produk, c.nama_produk, c.min_stok, c.max_stok, b.qty, b.qty_po, 
