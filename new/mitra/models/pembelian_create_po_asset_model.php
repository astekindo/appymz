...
class Pembelian_create_po_asset_model extends MY_Model {
	 ...
	 * @lastedited 2 jan 2012
	 */
	public function get_ro_detail_by_supplier($kd_supplier = '', $wkt_top = 0){
		$sql = "SELECT a.*, b.*, d.nama_produk, d.kd_peruntukkan, e.nm_satuan, f.nama_supplier, f.pic, f.alamat, g.waktu_top
				...
				JOIN mst.t_supplier f On f.kd_supplier = a.kd_supplier
				JOIN mst.t_supp_per_brg g ON g.kd_supplier = a.kd_supplier AND g.kd_produk = b.kd_produk
				WHERE a.kd_supplier = '".$kd_supplier."' AND a.no_ro like '".PREFIX_ASSET_REQUEST."%' AND a.konsinyasi='0' AND b.status = '2' AND g.waktu_top =".$wkt_top;
		$query = $this->db->query($sql);
                //print_r($sql);
		$rows = array();
		if($query->num_rows() > 0){
	...
	public function get_data_print($no_po = ''){	
		$sql = "select 'PURCHASE ORDER' title, a.no_po, a.tanggal_po, a.order_by_po, a.top,b.nama_supplier, a.alamat_kirim_po, a.remark,
					...
					a.tgl_berlaku_po, a.rp_dp, a.kirim_po, a.cetak_ke, a.approval_by
	...
	public function validate_po_by_kd_produk($kd_produk = ''){
