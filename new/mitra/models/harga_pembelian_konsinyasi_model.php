...
class Harga_pembelian_konsinyasi_model extends MY_Model {
	...
	public function search_produk_by_supplier($kd_supplier = "", $no_bukti = "", $kd_kategori1 = "", $kd_kategori2 = "", $kd_kategori3 = "", $kd_kategori4 = "", $kd_ukuran ="",$kd_satuan ="",$tgl_start ="",$tgl_end ="", $list = "", $search = '', $offset, $length){
		$where = ' WHERE ';

//		if ($kd_supplier != ''){
			$where .= "  a.kd_supplier = '$kd_supplier'";
                        $where .= "  AND b.is_konsinyasi = '1'";
//		}
		if ($no_bukti != ''){
...
//		}
	...
	public function search_produk_history($no_bukti = '', $kd_produk = ''){
		...
		$sql = "SELECT COALESCE(a.updated_date, a.created_date, a.updated_date) as tanggal, a.*,b.pct_margin, b.rp_ongkos_kirim, b.rp_het_harga_beli,b.rp_het_harga_beli_dist,nama_produk, nm_satuan 
					...
					JOIN mst.t_satuan c 
						ON c.kd_satuan = b.kd_satuan 
					WHERE 1=1 AND kd_peruntukan ='1' ".$where."
					ORDER BY b.nama_produk";
		$query = $this->db->query($sql);
		$rows = array();
		if($query->num_rows() > 0){
	...
	public function get_no_bukti_filter($kd_supplier ="",$search = "", $offset, $length){			
			$sql = "SELECT DISTINCT no_bukti AS no_bukti_filter, nama_supplier, a.keterangan, a.created_by
                                FROM mst.t_supp_per_brg_temp a, mst.t_supplier b, mst.t_produk c
                                WHERE b.kd_supplier=a.kd_supplier and a.status =  '0'
                                and a.kd_produk = c.kd_produk and c.is_konsinyasi = '1'
                                AND a.kd_peruntukan =  '0' AND b.kd_supplier ='$kd_supplier'
                                union
                                SELECT DISTINCT no_bukti AS no_bukti_filter, nama_supplier, x.keterangan, x.created_by
                                FROM mst.t_supp_per_brg x, mst.t_supplier y, mst.t_produk z
                                WHERE y.kd_supplier = x.kd_supplier 
                                and x.kd_produk = z.kd_produk and z.is_konsinyasi = '1'
                                AND x.tgl_end_diskon >= current_date AND x.kd_supplier ='$kd_supplier'";
			...
			$this->db->flush_cache();
                        $sql2 = "SELECT count(DISTINCT no_bukti) AS total
                                FROM mst.t_supp_per_brg_temp a, mst.t_supplier b, mst.t_produk c
                                WHERE b.kd_supplier=a.kd_supplier and a.status =  '0'
                                and a.kd_produk = c.kd_produk and c.is_konsinyasi = '1'
                                AND a.kd_peruntukan =  '0' 
                                union
                                SELECT count(DISTINCT no_bukti) AS no_bukti_filter
                                FROM mst.t_supp_per_brg x, mst.t_supplier y, mst.t_produk z
                                WHERE y.kd_supplier = x.kd_supplier 
                                and x.kd_produk = z.kd_produk and z.is_konsinyasi = '1'
                                AND x.tgl_end_diskon >= current_date";
	...
	public function update_temp($kd_supplier = '', $kd_produk = '', $no_bukti = '', $datau = NULL){
		$this->db->where('kd_supplier',$kd_supplier);
		$this->db->where('kd_produk',$kd_produk);
		...
		$this->db->where('tgl_start_diskon',$tgl_start_diskon);
         ...
         public function select_data_beli_temp($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = "",$no_bukti_filter =""){
             $where ="";
             if ($no_bukti_filter !=''){
                 $where.="AND no_bukti not in('$no_bukti_filter') AND kd_produk = '$kd_produk'";
             }else{
                if ($kd_produk !=''){
                 $where.= "AND kd_produk ='$kd_produk' and status = 0";
                } 
             }
                $sql = "select * from mst.t_supp_per_brg_temp
                          where tgl_start_diskon <= '$tgl_start_diskon' and tgl_end_diskon >= '$tgl_start_diskon' 
                          $where";
		
                $query = $this->db->query($sql);
                return $query->result();
	}
         public function select_data_beli($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = "",$no_bukti_filter =""){
             $where ="";
             if ($no_bukti_filter !=''){
                 $where.="AND no_bukti not in('$no_bukti_filter') AND kd_produk ='$kd_produk'";
             }else{
        ...
        public function select_data_end_temp($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = "", $no_bukti_filter =""){
		$where ="";
                if ($no_bukti_filter !=''){
                    $where.="AND no_bukti not in('$no_bukti_filter') AND kd_produk ='$kd_produk'";
                }else{
                ...
                $sql = "select * from mst.t_supp_per_brg_temp
                        where tgl_start_diskon <= '$tgl_end_diskon' and tgl_end_diskon >= '$tgl_end_diskon'
                        $where";
		
                $query = $this->db->query($sql);
                return $query->result();
	}
        public function select_data_end($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = "", $no_bukti_filter =""){
		$where ="";
                if ($no_bukti_filter !=''){
                    $where.="AND no_bukti not in('$no_bukti_filter') AND kd_produk ='$kd_produk'";
                }else{
	...
	public function select_data($kd_produk = "", $no_bukti_filter =""){
		
                $sql = "select * from mst.t_supp_per_brg
                          where kd_produk ='$kd_produk' and no_bukti not in ('$no_bukti_filter')
                          ";
