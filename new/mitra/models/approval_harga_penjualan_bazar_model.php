...
class Approval_harga_penjualan_bazar_model extends MY_Model {
	...
	public function search_produk_by_no_bukti($no_bukti = "", $search = '', $offset, $length){
		...
		$sql = "SELECT  a.*, b.kd_produk_lama, b.nama_produk, nm_satuan, 'Approve' as status
					FROM mst.t_diskon_sales_bazar_temp a
					JOIN mst.t_produk b
		...
		$this->db->flush_cache();
		$sql2 = "select count(*) as total FROM mst.t_diskon_sales_bazar_temp a
					JOIN mst.t_produk b
						ON b.kd_produk = a.kd_produk
					JOIN mst.t_satuan c
						ON c.kd_satuan = b.kd_satuan
	...
	public function get_no_bukti_filter($search = "", $offset, $length){
		...
		$query = $this->db->get("mst.t_diskon_sales_bazar_temp a", $length, $offset);
		//print_r($this->db->last_query());exit;
		$rows = array();
		if($query->num_rows() > 0){
			$rows = $query->result();
	...
	public function update_temp($no_bukti = '', $kd_produk = '', $status = '', $tanggal ='', $update_by =''){
		...
		$query =  $this->db->update('mst.t_diskon_sales_bazar_temp',$datau);
                //print_r($this->db->last_query());exit;
		return $query;
	}
	...
	public function insert_rows_diskon($datau = NULL){
		return $this->db->insert('mst.t_diskon_sales_bazar',$datau);
		// print_r($this->db->last_query());
