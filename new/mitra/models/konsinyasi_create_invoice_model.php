...
class Konsinyasi_create_invoice_model extends MY_Model {
	...
	public function insert_row($table = '', $data = NULL){
		...
		$sql = "UPDATE inv.t_hpp_inventory SET type = '5',
				...
				WHERE kd_produk = '$kd_produk' AND kd_peruntukan = '$kd_peruntukan'";
	...
	public function get_po_detail($no_po=''){
		$sql = "select c.no_do, a.no_po, d.kd_produk, d.nama_produk, e.qty_terima qty, nm_satuan, b.price_supp_po price_list,
				disk_amt_supp1_po, disk_amt_supp2_po, disk_amt_supp3_po, disk_amt_supp4_po,
				rp_disk_po, dpp_po, b.rp_total_po,
	...
	public function search_no_do_by_supplier($kd_supplier = '',$no_do = '',$search =''){
		if ($search != "") {
                 ...
                 $sql="select distinct c.* 
                    from purchase.t_purchase c 
                    join purchase.t_purchase_detail b on b.no_po = c.no_po
                    left join purchase.t_invoice_detail a on a.no_do = b.no_po and a.kd_produk = b.kd_produk 
                    where c.kd_peruntukan = '0' and c.kd_suplier_po = '$kd_supplier' and c.konsinyasi = '1' 
                    and a.no_do is null and a.kd_produk is null and c.no_po like 'PI%' ".$sql_search."";
	...
	public function search_no_do_by_supplier_no_do($kd_supplier = '', $no_do = ''){
		...
		$sql = "select c.no_po, d.kd_produk, e.nm_satuan, c.qty_po qty_terima, 
                        ...
                        where b.kd_suplier_po = '$kd_supplier'
                        and b.no_po in ($no_do)
                        and b.no_po = c.no_po
	...
	public function search_produk_by_supplier($kd_supplier = ''){
		...
		$sql = "SELECT a.*,b.*,nama_produk, nm_satuan, e.tanggal_po,f.pkp
