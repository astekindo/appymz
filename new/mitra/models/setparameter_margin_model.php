...
class Setparameter_margin_model extends MY_Model {
	...
	public function input_data($data = NULL){
		$result = $this->db->insert('mst.t_set_margin_markup', $data);
	...
	public function search_produk_by_kategori( $kd_kategori1 = "", $kd_kategori2 = "", $kd_kategori3 = "", $kd_kategori4 = "", $search = ''){
		...
		if ($search != ''){
			$where .= " AND ((lower(nama_produk) LIKE '%" . strtolower($search) . "%') OR (lower(kd_produk_lama) LIKE '%" . strtolower($search) . "%') OR (b.kd_produk LIKE '%" . $search . "%'))";
		}
		if ($kd_kategori1 != ''){
		...
		$sql = "SELECT
                ...
                , b.disk_amt_kons4
                , b.disk_amt_kons5
                , h.margin parameter_margin
                , h.margin_op parameter_margin_rp
                , h.markup parameter_markup
                , h.markup_op parameter_markup_rp
            FROM mst.t_produk a
            ...
            LEFT JOIN mst.t_set_margin_markup h
