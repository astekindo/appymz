...
class Konsinyasi_receive_order_by_po_model extends MY_Model {
	...
	public function get_all_po($kd_supplier = "", $search = ""){
		if($search != ''){
			$this->db->where("((lower(no_po) LIKE '%" . $search . "%') OR (no_po LIKE '%" . $search . "%'))", NULL);
		}
	...
	public function get_po_detail($no_po = '', $search = '', $sender = ''){
		...
		$this->db->select(" a.kd_produk, a.qty_po,a.qty_retur,
                ...
                $this->db->group_by(array("a.kd_produk","a.qty_retur", "a.no_po", "a.qty_po", " b.nama_produk", " b.kd_produk_supp", " b.kd_produk_lama", " c.nm_satuan", " e.kd_supplier", " e.nama_supplier"));
