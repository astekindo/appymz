...
class Harga_penjualan_distribusi_model extends MY_Model {
	...
	public function insert_rows_diskon_history($kd_produk = '', $kd_diskon_sales = '', $koreksi_ke = ''){
	 ...
	 public function select_data_dist($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = ""){
      ...
      public function select_data_dist_end($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = ""){
        ...
        public function select_data_dist_sama($kd_produk = "",$tgl_start_diskon = "",$tgl_end_diskon = ""){
	...
	public function search_produk_by_kategori($kd_supplier = "", $no_bukti = "", $konsinyasi = "", $kd_kategori1 = "", $kd_kategori2 = "", $kd_kategori3 = "", $kd_kategori4 = "",$kd_ukuran = "",$kd_satuan = "", $list = "", $search = '', $offset, $length){
		...
		if ($no_bukti != ''){
			$select = " i.*,b.rp_het_harga_beli,b.rp_het_harga_beli_dist, ";
			$from = "JOIN mst.t_diskon_sales_dist_temp i ON b.kd_produk = i.kd_produk  ";
		...
		$sql = "SELECT b.kd_produk kd_produk_baru, b.kd_produk_lama,b.nama_produk,b.rp_cogs_dist p_rp_cogs, b.rp_het_cogs_dist p_rp_het_cogs, " . $select . " h.hrg_supplier, h.net_hrg_supplier_sup_inc, h.net_hrg_supplier_dist_inc,a.koreksi_ke koreksi_diskon, b.koreksi_ke koreksi_produk, nm_satuan, 
				(SELECT nama_supplier FROM mst.t_supplier s WHERE s.kd_supplier = h.kd_supplier) nama_supplier
					...
					LEFT JOIN (select * from mst.t_diskon_sales_dist  where (tgl_start_diskon <= current_date or tgl_start_diskon is null) and (tgl_end_diskon >= current_date or tgl_end_diskon is null)) a
					...
					WHERE h.hrg_supplier_dist > 0 AND b.aktif = 1 ".$where."
		...
		$sql2 = "select count(*) as total FROM mst.t_produk b
					...
					JOIN mst.t_supp_per_brg h 
						ON h.kd_produk = b.kd_produk
					LEFT JOIN mst.t_diskon_sales_dist a
						ON b.kd_produk = a.kd_produk 
					JOIN mst.t_satuan c 
						ON c.kd_satuan = b.kd_satuan 
					JOIN mst.t_kategori1 d
	...
	public function search_produk_history($no_bukti = '',$kd_produk = ''){
		...
		$sql = "SELECT a.*,b.*, d.nama_supplier, h.hrg_supplier, h.net_hrg_supplier_sup_inc,h.net_hrg_supplier_dist_inc, a.tanggal, a.koreksi_ke koreksi_diskon, nm_satuan 
					FROM mst.t_produk b 
					JOIN mst.t_supp_per_brg h ON h.kd_produk = b.kd_produk
					LEFT JOIN mst.t_diskon_sales_dist a ON b.kd_produk = a.kd_produk 
	...
	public function get_data_print($no_bukti = '', $kd_produk = ''){
		$where = '';
		$title = 'HISTORY HARGA PENJUALAN DISTRIBUSI';
	...
	public function search_no_bukti($kd_supplier = "",$search = "", $offset, $length){
        ...
        public function search_no_bukti_approve($kd_supplier = "",$search = "", $offset, $length){
			$this->db->select("a.keterangan");
			$this->db->select("a.created_by");
			$this->db->select("b.nama_supplier");
			$this->db->select("a.kd_diskon_sales");
	...
	public function insert_temp($data = NULL){
		return $this->db->insert('mst.t_diskon_sales_dist_temp', $data);
	}
}
