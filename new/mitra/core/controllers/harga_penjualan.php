		...
		foreach($detail as $obj){
			if($obj->edited == 'Y'){
				...
				if($no_bukti_filter != ''){
					...
					if($this->hj_model->update_temp($kd_produk, $no_bukti_filter,$diskon_hj)){
						$results = 'success';
					}else{
						$this->db->trans_rollback();
						...
						exit;
                                        ...
                                        $result = $this->hj_model->select_temp($kd_produk,'0'); 
					if(!empty($result)){
						$this->db->trans_rollback();
						echo '{"success":false,"errMsg":"Barang dengan Kode Barang: '.$kd_produk.' Belum Diapprove"}';
						exit;
					// }else if($this->hj_model->select_temp($kd_produk,'1')){
				...
				// if(!$kd_diskon_sales){
					// $no_urut_diskon = $this->hj_model->get_kode_sequence('HJUAL',3);			
					// $kd_diskon_sales = date('Ym').'-'.$no_urut_diskon;
					// $diskon_hj['kd_produk']	=	$kd_produk;
					// $diskon_hj['kd_diskon_sales']	=	$kd_diskon_sales;
					// $diskon_hj['created_by']	=	$updated_by;
					// $diskon_hj['created_date']	=	$updated_date;
					
					// if($this->hj_model->insert_rows_diskon($diskon_hj)){
						// if($this->hj_model->insert_rows_diskon_history($kd_produk, $kd_diskon_sales, $koreksi_diskon)){
							// $result_disk++;
						// }
					// }else{
					...
					// }
				// }else{
					// $diskon_hj['updated_by']	=	$updated_by;
					// $diskon_hj['updated_date']	=	$updated_date;
					
					// if($this->hj_model->update_rows_diskon($kd_produk, $kd_diskon_sales, $diskon_hj)){
						// if($this->hj_model->insert_rows_diskon_history($kd_produk, $kd_diskon_sales, $koreksi_diskon)){
							// $result_disk++;
						// }
					// }else{
				...
				// if($this->hj_model->update_rows_produk($kd_produk, $produk_hj)){
					// if($this->hj_model->insert_rows_produk_history($kd_produk, $koreksi_produk)){
						// $result_prod++;
					// }
				// }else{
					// echo '{"success":false,"errMsg":"Produk Failed"}';
					// $this->db->trans_rollback();
					// exit;
				// }
