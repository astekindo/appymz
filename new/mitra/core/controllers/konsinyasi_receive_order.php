...
class Konsinyasi_receive_order extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('konsinyasi_receive_order_model', 'kro_model');
    }
    ...
    public function get_form() {
        // $no_do = 'RK' . date('Ymd') . '-';
        // $sequence = $this->kro_model->get_kode_sequence($no_do, 3);
        echo '{"success":true,
				"data":{
					"no_do":"",
					"tanggal":"' . date('d-M-Y') . '",
    ...
    public function update_row() {
        ...
        if (count($detail) > 0) {
            ...
            $header_result = $this->kro_model->insert_row('purchase.t_receive_order', $header_do);
            ...
            foreach ($detail as $obj) {
                ...
                $detail_do['jumlah_barcode'] = $obj->jumlah_barcode;
                ...
                if ($this->kro_model->insert_row('purchase.t_dtl_receive_order', $detail_do)) {
                    $detail_result++;

                    $sum_qty_terima = $this->kro_model->get_sum_qty_terima($obj->no_po, $obj->kd_produk);

                    //if ($obj->qty_po == $sum_qty_terima->sum) {

                    //    $sql = "UPDATE purchase.t_purchase SET close_po = 1 WHERE no_po = '" . $obj->no_po . "'";
                    //    $this->kro_model->query_update($sql);
                    //}
                    ...
                    $this->kro_model->insert_row('inv.t_trx_inventory', $trx_inventory);

                    if ($this->kro_model->get_stok_inventory($obj->kd_produk)) {
                        ...
                        $this->kro_model->insert_row("inv.t_brg_inventory", $brg_inventory);
                    }

                    $hpp_result = $this->kro_model->get_hpp_by_kd_produk($obj->kd_produk, $obj->no_po);
                    ...
                    $cogs_result = $this->kro_model->select_cogs($obj->kd_produk);
                    ...
                    if ($this->kro_model->get_hpp_inventory($obj->kd_produk)) {
                        unset($hpp_inventory);
                        ...
                        $this->kro_model->update_row_hpp($hpp_result->kd_peruntukan, $obj->kd_produk, $hpp_inventory);
                    } else {
                        unset($hpp_inventory);
                        ...
                        $hpp_inventory['rp_het_hrg_beli'] = $hpp_het_hrg_beli;
                        $hpp_inventory['tanggal'] = date('Y-m-d H:i:s');
                        $this->kro_model->insert_row("inv.t_hpp_inventory", $hpp_inventory);
                    }


                    unset($hpp_inventory_histo);
                    $hpp_inventory_histo['no_bukti'] = "MB" . date('Ym') . '-' . $this->kro_model->get_kode_sequence("MB" . date('Ym'), 5);
                    ...
                    $this->kro_model->insert_row("inv.t_hpp_inventory_histo", $hpp_inventory_histo);

                    $this->kro_model->update_row_produk($hpp_result->kd_peruntukan, $obj->kd_produk, $hpp_cogs, $hpp_het);
                }
            }
        }
        $title = 'RECEIVE ORDER';
    ...
    public function get_all_po() {
        ...
        $result = $this->kro_model->get_all_po($kd_supplier, $search);
    ...
    public function get_po_detail() {
        ...
        $result = $this->kro_model->get_po_detail($no_po);
    ...
    public function search_supplier() {
        ...
        $result = $this->kro_model->search_supplier($search, $start, $limit);
    ...
    public function search_ekspedisi() {
        ...
        $result = $this->kro_model->search_ekspedisi($search, $start, $limit);
    ...
    public function search_satuan() {
        ...
        $result = $this->kro_model->search_satuan($search, $start, $limit, $kd_ekspedisi);
    ...
    public function search_produk_by_no_po() {
        ...
        $result = $this->kro_model->get_po_detail($no_po, $search, $sender);
    ...
    public function print_form($no_do = '', $title = '') {
        ...
        $data = $this->kro_model->get_data_print($no_do, $title);
        if (!$data)
            show_404('page');
    ...
    public function get_rows_lokasi() {
        ...
        $result = $this->kro_model->get_rows_lokasi($kd_produk, $search, $start, $limit);
