...
class Pembelian_receive_order extends MY_Controller {
		...
		if(count($detail) > 0){	
			...
			foreach($detail as $obj){
				...
				if($this->pro_model->insert_row('purchase.t_dtl_receive_order', $detail_do)){
					...
					$sum_qty_terima = $this->pro_model->get_sum_qty_terima($obj->no_po, $obj->kd_produk);
					...
					if($this->pro_model->get_stok_inventory($obj->kd_produk)){
					...
					$hpp_result = $this->pro_model->get_hpp_by_kd_produk($obj->kd_produk,$obj->no_po);
					...
					$cogs_result = $this->pro_model->select_cogs($obj->kd_produk);
					...
					if($this->pro_model->get_hpp_inventory($obj->kd_produk)){
						unset($hpp_inventory);
						...
						$this->pro_model->update_row_hpp($hpp_result->kd_peruntukan, $obj->kd_produk,$hpp_inventory);
					}else{
						unset($hpp_inventory);
					...
					$this->pro_model->update_row_produk($hpp_result->kd_peruntukan, $obj->kd_produk, $hpp_cogs, $hpp_het);
	...
	public function get_all_po(){
		...
		$result = $this->pro_model->get_all_po($kd_supplier, $search);
	...
	public function get_rows_lokasi(){
        ...
        $result = $this->pro_model->get_rows_lokasi($kd_produk, $search, $start, $limit);
