...
class Pembelian_create_po_non_request extends MY_Controller {
	...
	public function search_produk_by_supplier(){			
		...
		foreach($result as $obj){
			...
			if(!empty($obj->disk_persen_supp1) && $obj->disk_persen_supp1 > 0){
				$diskon_supp1 = ($obj->disk_persen_supp1 * $obj->hrg_supplier) /100;
				$diskon_supp1_view = $obj->disk_persen_supp1." %";
			}else{
				if(!empty($obj->disk_amt_supp1) && $obj->disk_amt_supp1 > 0){
			...
			if(!empty($obj->disk_persen_supp4) && $obj->disk_persen_supp4 > 0){
				...
				if(!empty($obj->disk_amt_supp4) && $obj->disk_amt_supp4 > 0){
		...
		if($action == 'validate'){
			...
			$validate = $this->pcpo_model->validate_po_by_kd_produk($kd_produk);
			if($validate->sum != 0){
