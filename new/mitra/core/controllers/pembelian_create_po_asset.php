...
class Pembelian_create_po_asset extends MY_Controller {
	/**
	 * @author dhamarsu
	 * @editedby luxse
    ...
    public function __construct() {
        parent::__construct();
		$this->load->model('pembelian_create_po_asset_model', 'pcpa_model');
    ...
    public function get_form(){
    	// $no_po = 'PA' . date('Ymd') . '-';
    	// $sequence = $this->pcpa_model->get_kode_sequence($no_po, 3);
		$jth_tempo_po = $this->pcpa_model->get_jth_tempo_po();
    ...
    public function update_row(){
		...
		$header_result = $this->pcpa_model->insert_row('purchase.t_purchase', $header_po);
		...
		foreach($detail as $obj){
			...
			$detail_pr['disk_persen_supp1_po'] = (int) $obj->disk_persen_supp1_po;
			$detail_pr['disk_persen_supp2_po'] = (int) $obj->disk_persen_supp2_po;
			$detail_pr['disk_persen_supp3_po'] = (int) $obj->disk_persen_supp3_po;
			$detail_pr['disk_persen_supp4_po'] = (int) $obj->disk_persen_supp4_po;
			...
			$detail_pr['rp_disk_po'] = $obj->disk_persen_supp1 + $obj->disk_persen_supp2 + $obj->disk_persen_supp3 + $obj->disk_persen_supp4 + $obj->disk_persen_supp5;
			...
			if($this->pcpa_model->insert_row('purchase.t_purchase_detail', $detail_pr)){
				$this->pcpa_model->update_detail_request_order($obj->no_ro, $obj->kd_produk, (int) $obj->qty);
				$detail_result++;
			}
		...
		if ($header_result && $detail_result > 0) {
			...
			$result = '{"success":true,"errMsg":"Pembuatan PO Asset berhasil, NO PA: '.$no_po.' ","printUrl":"' . site_url("pembelian_create_po/print_form/" . $no_po . $sequence) . '"}';
		} else {
	...
	public function get_row_harga_supplier(){
		...
		$result = $this->pcpa_model->get_row_harga_supplier($search_by, $id, $kd_produk);
	...
	public function search_supplier(){			
		...
		$result = $this->pcpa_model->search_supplier($search, $start, $limit);
	...
	public function get_all_ro(){
		...
		$result = $this->pcpa_model->get_all_ro($keyword);
	...
	public function get_supplier_by_barang(){
		...
		if($cmd == 'get'){
			$result = $this->pcpa_model->get_supplier_by_barang($kd_produk);
		}else{
	...
	public function get_ro_detail_by_supplier($kd_supplier = '', $wkt_top = '', $pkp = 1){
		...
		$detail = $this->pcpa_model->get_ro_detail_by_supplier($kd_supplier, $wkt_top);
		
		$data = array();
		foreach($detail as $obj){
			
			$result = $this->pcpa_model->get_row_harga_produk_per_supplier($obj->kd_produk, $obj->kd_supplier);
			...
			if(count($result) > 0){
				...
				if($result->disk_persen_supp1 != '' && $result->disk_persen_supp1 != 0){
					$diskon_supp1 = ($result->disk_persen_supp1 * $result->hrg_supplier) /100;
					$disk_grid_supp1 = number_format($result->disk_persen_supp1).'%';
				}else{
				...
				if($result->disk_persen_supp2 != '' && $result->disk_persen_supp2 != 0){
					...
					$disk_grid_supp2 = number_format($result->disk_persen_supp2).'%';
				}else{
				...
				if($result->disk_persen_supp3 != '' && $result->disk_persen_supp3 != 0){
					...
					$disk_grid_supp3 = number_format($result->disk_persen_supp3).'%';
				}else{
				...
				if($result->disk_persen_supp4 != '' && $result->disk_persen_supp4 != 0){
					...
					$disk_grid_supp4 = number_format($result->disk_persen_supp4).'%';
				}else{
	...
	public function get_ro_detail($no_ro = '', $wkt_top = ''){
		$detail = $this->pcpa_model->get_ro_detail($no_ro);
	...
	public function get_all_produk(){
		$result = $this->pcpa_model->get_all_produk();
	...
	public function get_row_produk(){
		$id = isset($_POST['id']) ? $this->db->escape_str($this->input->post('id',TRUE)) : NULL;
        $result = $this->pcpa_model->get_row_produk($id);
	...
	public function get_term_of_payment_by_supplier($kd_supplier = ''){
		$result = $this->pcpa_model->get_term_of_payment_by_supplier($kd_supplier);
	...
	public function get_term_of_payment($no_ro = ''){
		...
		$result = $this->pcpa_model->get_term_of_payment($no_ro);
	...
	public function print_form($no_po = ''){
		$this->pcpa_model->setCetakKe($no_po);

		$data = $this->pcpa_model->get_data_print($no_po);
	...
	public function get_nilai_parameter_pic(){
		$result = $this->pcpa_model->get_nilai_parameter(PIC_PENERIMA_PO);
		return $result;
	}
	
	public function get_nilai_parameter_alamat(){
		$result = $this->pcpa_model->get_nilai_parameter(ALAMAT_PENERIMA_PO);
		return $result; 
	}
}
