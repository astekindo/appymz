...
class Konsinyasi_create_po_non_request extends MY_Controller {
    ...
    public function update_row(){
		...
		if ($header_result && $detail_result > 0) {
			...
			$result = '{"success":true,"errMsg":"Pembuatan PO Non PR Konsinyasi Berhasil, NO PS: '.$no_po.'","printUrl":"' . site_url("konsinyasi_create_po_non_request/print_form/" . $no_po . $sequence) . '"}';
		} else {
			$this->db->trans_rollback();
	...
	public function search_produk_by_supplier(){
		...
		$result = $this->kcpnr_model->search_produk_by_supplier($kd_supplier,$sender,$waktu_top, $search, $start, $limit);
