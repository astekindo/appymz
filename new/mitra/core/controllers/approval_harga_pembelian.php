...
class Approval_harga_pembelian extends MY_Controller {
	...
	public function approval(){
			...
			if($obj->status == 'Approve'){
				if($obj->edited == 'Y'){
					...
					$detail_hp['no_bukti']	=	$no_bukti;
                                        $detail_hp['is_konsinyasi']	=	$is_konsinyasi;
		...
		foreach($result as $data)
		{
			...
			if($status_approve == '1'){
				if($this->ahp_model->update_row($kd_supplier, $kd_produk, $waktu_top, $data)){
					...
					if($this->ahp_model->update_net_produk($kd_produk,$rp_het_harga_beli,$rp_het_harga_beli_dist,$data['net_hrg_supplier_sup'],$data['net_hrg_supplier_dist'])){
						$results = 'success';
					}else{
						$this->db->trans_rollback();
					...
					if($diskon_sales_result){
						if($this->ahp_model->update_hrgJual_produk($kd_produk,$rp_het_harga_beli,$rp_het_harga_beli_dist,$data['net_hrg_supplier_sup'])){
							$results = 'success';
						}else{
							$this->db->trans_rollback();
							echo '{"success":false,"errMsg":"update_net_produk Failed . . ."}';
