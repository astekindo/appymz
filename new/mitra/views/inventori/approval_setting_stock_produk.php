...
<script type="text/javascript">  
    ...
    // data store
    var strassp = new Ext.data.Store({
        ...
        proxy: new Ext.data.HttpProxy({
            url: '<?= site_url("approval_setting_stock/get_rows_detail") ?>',
            method: 'POST'
        }),
        listeners: {
    ...
    action_approval_detail_approve_manager.on('action', function(grid, record, action, row, col) {
		var kd_prod_det = record.get('kd_produk');
		switch(action){
			case 'icon-approve-record':
				edit_approval_setting_stok_p(kd_prod_det,1);
			break;
		}		
    });
    ...
    action_approval_detail_reject_manager.on('action', function(grid, record, action, row, col) {
		...
		switch(action){
			case 'icon-delete-record' :
				edit_approval_setting_stok_p(kd_prod_det,9);
			break;
		}		
    });  
	...
	var cmm = new Ext.ux.grid.LockingColumnModel({
        ...
        columns: [ {
            ...
            dataIndex: 'kd_produk',
            hidden: true
        },action_approval_detail_approve_manager,action_approval_detail_reject_manager,{
            header: "Kode Barang",
            dataIndex: 'kd_produk',
            sortable: true,
            width: 150
	...
	var gridASSP = new Ext.grid.EditorGridPanel({
		id: 'gridASSP',
        store: strassp,
        stripeRows: true,
	...
	var approvalssp = new Ext.FormPanel({
			id: 'approvalssp',
			border: false,
			frame: true,
			autoScroll:true,		
	...
	function edit_approval_setting_stok_p(kd_prod_det,stat){
        var sm = gridASSP.getSelectionModel();
            ...
            Ext.Msg.show({
                ...
                fn: function(btn){
                    if (btn == 'yes') {
                        ...
                        var data =  kd_prod_det +'_'+ stat;                        
                        Ext.Ajax.request({
                            url: '<?= site_url("approval_setting_stock/update_row_detail") ?>',
                            method: 'POST',
                            params: {
                                postdata : data
                            },
