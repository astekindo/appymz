...
//-------- COMBOBOX KATEGORI ---------------------
    ...
    var strPVCPK1Kategori = new Ext.data.ArrayStore({
        fields: ['kd_kategori1', 'nama_kategori1'],
        data: []
    });

    // GRID PANEL TWIN COMBOBOX kategori1 Data Store
    var strGridPVCPK1Kategori = new Ext.data.Store({
    ...
    strGridPVCPK1Kategori.on('load', function(){
        Ext.getCmp('id_searchGridPVCPK1Kategori').focus();
    });

    // SEARCH GRID PANEL TWIN COMBOBOX kategori1
    var searchGridPVCPK1Kategori = new Ext.app.SearchField({
        store: strGridPVCPK1Kategori,
        width: 350,
        id: 'id_searchGridPVCPK1Kategori'
    });

    // GRID PANEL TWIN COMBOBOX kategori1
    var GridPVCPK1Kategori = new Ext.grid.GridPanel({
        store: strGridPVCPK1Kategori,
        stripeRows: true,
        frame: true,
        border: true,
        ...
        tbar: new Ext.Toolbar({
            items: [searchGridPVCPK1Kategori]
        })
    });

    var menuPVCPK1Kategori = new Ext.menu.Menu();

    menuPVCPK1Kategori.add(new Ext.Panel({
        ...
        closeAction: 'hide',
        plain: true,
        items: [GridPVCPK1Kategori],
        buttons: [{
            text: 'Done',
            handler: function () {
                var sm  = GridPVCPK1Kategori.getSelectionModel();
    ...
    Ext.ux.TwinComboSuplier = Ext.extend(Ext.form.ComboBox, {
        ...
        onTrigger2Click: function () {
            //load store grid
            strGridPVCPK1Kategori.load();
            menuPVCPK1Kategori.showAt([this.getPosition()[0], this.getPosition()[1] + this.getHeight()]);
        },
    ...
    menuPVCPK1Kategori.on('hide', function () {
        var sf = Ext.getCmp('id_searchGridPVCPK1Kategori').getValue();
        if (sf != '') {
            Ext.getCmp('id_searchGridPVCPK1Kategori').setValue('');
            searchGridPVCPK1Kategori.onTrigger2Click();
        }
    });
    ...
    var comboPVCPK1Kategori = new Ext.ux.TwinComboSuplier({
        fieldLabel: 'Kategori',
        id: 'id_cbPVCPK1Kategori',
        store: strPVCPK1Kategori,
        mode: 'local',
...
//-------- CHECKBOX SORT ORDER -------------------
    var sortPVCPK1 = new Ext.form.Checkbox({
        xtype: 'checkbox',
...
//-------- HEADER FORM ---------------------------
    var headerPVCPK1Utama = {
        buttonAlign: 'left',
        layout: 'form',
        border: false,
        labelWidth: 100,
        defaults: { labelSeparator: ''},
        items: [headerPVCPK1Tanggal, headerPVCPK1Kategori],
        buttons: [{
            text: 'Print',
            formBind: true,
            handler: function () {
                Ext.getCmp('rpt_penjualan_vs_cogs_perkategori1').getForm().submit({
                    url: '<?= site_url("laporan_penjualan_vs_cogs_per_kategori1/get_report") ?>',
                    scope: this,
                    waitMsg: 'Saving Data...',
