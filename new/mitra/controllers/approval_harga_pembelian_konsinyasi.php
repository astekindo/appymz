...
class Approval_harga_pembelian_konsinyasi extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('approval_harga_pembelian_konsinyasi_model', 'ahpk_model');
        $this->load->model('harga_pembelian_model', 'hp_model');
        $this->load->model('pembelian_retur_model', 'pret_model');
        $this->load->model('pembelian_create_po_non_request_model', 'pcpnr_model');
    }
    ...
    public function search_produk_by_no_bukti() {
        ...
        $data_result = $this->ahpk_model->search_produk_by_no_bukti($no_bukti, $search, $start, $limit);
    ...
    public function get_no_bukti_filter() {
        ...
        $result = $this->ahpk_model->get_no_bukti_filter($search, $start, $limit);
    ...
    public function get_no_bukti_filter_dist() {
        ...
        $result = $this->ahpk_model->get_no_bukti_filter_dist($search, $start, $limit);
    ...
    public function approval() {
        ...
        foreach ($detail as $obj) {
           ...
           if ($obj->status == 'Approve') {
            ...
            if (!($this->ahpk_model->update_temp($no_bukti, $obj->kd_produk, $status))) {
            ...
            if ($obj->status == 'Approve') {
                // if ($obj->edited == 'Y') {
                    ...
                    //$detail_hp['tgl_end_diskon'] = $tgl_end_diskon;
        ...
        $result = $this->ahpk_model->get_data_temp($no_bukti);
        ...
        foreach ($result as $data) {
            ...
            //$tgl_end_diskon = $data['tgl_end_diskon'];
            // print_r($data);
            // exit;
            unset($data_hps);
            ...
            if ($status_approve == '1') {
                $produk = $this->ahpk_model->select_data_beli($kd_produk,$tgl_start_diskon);
                if ($produk){
                    //Update Harga
                    if ($this->ahpk_model->update_row($kd_produk,$tgl_start_diskon, $data_hps)) {
                           $results = 'success';
                    }else{
                        $this->db->trans_rollback();
                    ...
                    if ($this->ahpk_model->insert_row('mst.t_supp_per_brg', $data_hps)) {
                            $results = 'success';
                        }else {
                            $this->db->trans_rollback();
                ...
                if ($this->ahpk_model->update_harga_beli_non_aktif($kd_produk,$tgl_start_diskon)) {
                           $results = 'success';
                }else {
                        $this->db->trans_rollback();
                ...
                if ($this->ahpk_model->update_harga_beli_tgl_end($kd_produk,$tgl_start_diskon)) {
                           $results = 'success';
                }else {
                        $this->db->trans_rollback();
...
//                    $produk_result = $this->ahpk_model->get_produk_margin($kd_produk);
...
//
//                    if ($this->ahpk_model->update_net_produk($kd_produk, $rp_het_harga_beli, $data['net_hrg_supplier_sup'])) {
//                        $results = 'success';
//                    } else {
//                        $this->db->trans_rollback();
...
//                    }
                ...
                if($this->ahpk_model->is_auto_update_hj($kd_supplier)) {
                    $data_harga_jual = $this->ahpk_model->get_data_harga_jual($kd_produk,$tgl_start_diskon);
                    if ($data_harga_jual > 0){ //Update Harga Jual
                        $updatehj['disk_persen_kons1'] = $disk_persen_supp1;
                        $updatehj['disk_persen_kons2'] = 0;
                        $updatehj['disk_persen_kons3'] = 0;
                        $updatehj['disk_persen_kons4'] = 0;
                        $updatehj['disk_amt_kons1'] = $disk_amt_supp1;
                        $updatehj['disk_amt_kons2'] = 0;
                        $updatehj['disk_amt_kons3'] = 0;
                        $updatehj['disk_amt_kons4'] = 0;
                        $updatehj['disk_amt_kons5'] = 0;
                        $updatehj['disk_persen_member1'] = $disk_persen_supp1;
                        $updatehj['disk_persen_member2'] = 0;
                        $updatehj['disk_persen_member3'] = 0;
                        $updatehj['disk_persen_member4'] = 0;
                        $updatehj['disk_amt_member1'] = $disk_amt_supp1;
                        $updatehj['disk_amt_member2'] = 0;
                        $updatehj['disk_amt_member3'] = 0;
                        $updatehj['disk_amt_member4'] = 0;
                        $updatehj['disk_amt_member5'] = 0;
                        $inserthj['rp_cogs']= 0;
                        $inserthj['rp_het_cogs'] = 0;
                        $updatehj['rp_jual_supermarket'] = $hrg_supplier;
                        $updatehj['updated_by']= $this->session->userdata('username');
                        $updatehj['updated_date'] = date('Y-m-d H:i:s');
                        if($result = $this->ahpk_model->update_diskon_jual_konsinyasi($kd_produk,$tgl_start_diskon,$updatehj)){
                            $results = 'success';
                         }else {
                            $this->db->trans_rollback();
                            echo '{"success":false,"errMsg":"update diskon konsinyasi Failed . . ."}';
                            exit;
                        }
                    }else { //Insert Harga Jual
                        $rp_cogs = $net_hrg_supplier * 1.1;
                        $inserthj['kd_produk'] = $kd_produk;
                        $inserthj['kd_diskon_sales'] = $no_bukti;
                        $inserthj['tanggal'] = date('Y-m-d H:i:s');
                        $inserthj['disk_persen_kons1'] = $disk_persen_supp1;
                        $inserthj['disk_persen_kons2'] = 0;
                        $inserthj['disk_persen_kons3'] = 0;
                        $inserthj['disk_persen_kons4'] = 0;
                        $inserthj['disk_amt_kons1'] = $disk_amt_supp1;
                        $inserthj['disk_amt_kons2'] = 0;
                        $inserthj['disk_amt_kons3'] = 0;
                        $inserthj['disk_amt_kons4'] = 0;
                        $inserthj['disk_amt_kons5'] = 0;
                        $inserthj['disk_persen_member1'] = $disk_persen_supp1;
                        $inserthj['disk_persen_member2'] = 0;
                        $inserthj['disk_persen_member3'] = 0;
                        $inserthj['disk_persen_member4'] = 0;
                        $inserthj['disk_amt_member1'] = $disk_amt_supp1;
                        $inserthj['disk_amt_member2'] = 0;
                        $inserthj['disk_amt_member3'] = 0;
                        $inserthj['disk_amt_member4'] = 0;
                        $inserthj['disk_amt_member5'] = 0;
                        $inserthj['created_by']= $this->session->userdata('username');
                        $inserthj['created_date'] = date('Y-m-d H:i:s');
                        $inserthj['koreksi_ke'] = 1;
                        $inserthj['hrg_beli_sup'] = $net_hrg_supplier;
                        $inserthj['rp_het_harga_beli'] = $net_hrg_supplier_sup_inc;
                        $inserthj['rp_jual_supermarket'] = $hrg_supplier;
                        $inserthj['rp_cogs']= 0;
                        $inserthj['rp_het_cogs'] = 0;
                        $inserthj['keterangan']= 'Harga Jual Saat Update Harga Beli';
                        $inserthj['qty_member']= 0;
                        $inserthj['qty_beli_member']= 0;
                        $inserthj['qty_bonus']= 0;
                        $inserthj['qty_beli_bonus']= 0;
                        $inserthj['tgl_start_diskon'] = $tgl_start_diskon;
                        if($result = $this->ahpk_model->insert_row('mst.t_diskon_sales', $inserthj)){
                             $results = 'success';
                         }else {
                            $this->db->trans_rollback();
                            echo '{"success":false,"errMsg":"insert diskon konsinyasi Failed . . ."}';
                            exit;
                        }
                    }
                    //Non Aktifkan Harga Masa Depan
                    if ($this->ahpk_model->update_harga_jual_non_aktif($kd_produk,$tgl_start_diskon)) {
                               $results = 'success';
                    }else {
                            $this->db->trans_rollback();
                            echo '{"success":false,"errMsg":"Non Aktif Failed . . ."}';
                ...
                if ($this->ahpk_model->insert_history($data)) {
                    $results = 'success';
                } else {
                    $this->db->trans_rollback();
    ...
    public function approval_distribusi() {
        ...
        foreach ($result as $data) {
            ...
            if ($status_approve == '1') {
                if ($this->ahpk_model->update_row($kd_supplier, $kd_produk, $waktu_top, $data_hp)) {
                    ...
                    if ($this->ahpk_model->update_net_produk_dist($kd_produk, $rp_het_harga_beli_dist, $data['net_hrg_supplier_dist'])) {
                        $results = 'success';
                    } else {
                        $this->db->trans_rollback();
                        echo '{"success":false,"errMsg":"update_net_produk Failed . . ."}';
...
//					$diskon_sales_result = $this->ahpk_model->get_hrgJual_produk($kd_produk);
//					if($diskon_sales_result){
//						if($this->ahpk_model->update_hrgJual_produk_dist($kd_produk,$rp_het_harga_beli_dist,$data['net_hrg_supplier_dist'])){
//							$results = 'success';
//						}else{
//							$this->db->trans_rollback();
//							echo '{"success":false,"errMsg":"update_net_produk Failed . . ."}';
...
//					}
        ...
        if ($results == 'success') {
            if ($is_konsinyasi == '1') {
                ...
                $result = $this->ahpk_model->update_diskon_jual_konsinyasi($kd_produk, $updatehj);
            }
