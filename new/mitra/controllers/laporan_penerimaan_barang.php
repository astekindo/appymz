...
class Laporan_penerimaan_barang extends MY_Controller {
	/**
	 * @author dhamarsu
	 * @editedby luxse
	...
	public function search_supplier(){			
		...
		$result = $this->lpb_model->search_supplier($search, $start, $limit);
	...
	public function print_form($no_ro = ''){
		$data = $this->lpb_model->get_data_print($no_ro);
		if(!$data) show_404('page');
				
		$this->output->set_content_type("application/pdf");
		require_once(APPPATH . 'libraries/LaporanPenerimaanBarangPrint.php');
		$pdf = new LaporanPenerimaanBarangPrint(PDF_PAGE_ORIENTATION_LANDSCAPE, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$pdf->setKertas();
