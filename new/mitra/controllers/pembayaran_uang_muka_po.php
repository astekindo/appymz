...
class Pembayaran_uang_muka_po extends MY_Controller {  

    public function __construct() {
        parent::__construct();
		
                $this->load->model('Pembayaran_uang_muka_po_model','pumpo_model');
    }
	
	
    public function get_form(){
    	//$no_kwit = 'PPO' . date('Ym') . '-';
    	//$sequence = $this->pumpo_model->get_kode_sequence($no_kwit, 3);
    	echo '{"success":true,
				"data":{
    ...
    public function update_row(){
                ...
                $no_ret = 'UMPO' . $current_date . '-';
                $sequence = $this->pumpo_model->get_kode_sequence($no_ret, 3);
                $no_bukti = $no_ret . $sequence;
		...
		$total_bayar = isset($_POST['_total_bayar']) ? $this->db->escape_str($this->input->post('_total_bayar',TRUE)) : 0;
                $rp_bayar = isset($_POST['_rp_bayar']) ? $this->db->escape_str($this->input->post('_rp_bayar',TRUE)) : 0;
		...
		$header_ppo['keterangan'] = $keterangan;
                $header_ppo['rp_total_bayar'] = (int) $rp_bayar;
		
				 
		if( ! $this->pumpo_model->insert_row('purchase.t_pembayaran_po', $header_ppo)){
		...
		foreach($detail as $obj){
			...
			$detail_ppo['rp_bayar'] = (int) $obj->rp_pembayaran_uang_muka_po;
			
			
			if ($this->pumpo_model->insert_row('purchase.t_pembayaran_po_detail', $detail_ppo)){
                            $sql = "UPDATE purchase.t_purchase SET rp_dp = rp_dp + " . $obj->rp_pembayaran_uang_muka_po . " WHERE no_po = '" . $obj->no_po . "'";
                            $this->pumpo_model->query_update($sql);
                        }else {
                            echo '{"success":false,"errMsg":"Process Failed.."}';
				$this->db->trans_rollback();
				exit;
		...
		foreach($detailbayar as $obj){
			...
			if(! $this->pumpo_model->insert_row('purchase.t_pembayaran_po_bayar', $detail_bayar)){
				echo '{"success":false,"errMsg":"Process Failed.."}';
				$this->db->trans_rollback();
				exit;
			}
	...
	public function get_all_invoice(){
		$kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier',TRUE)) : "";
		$result = $this->pumpo_model->get_all_invoice($kd_supplier);
        
        echo $result;
	}
        public function get_no_po_bysupplier(){
		$kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier',TRUE)) : "";
		$result = $this->pumpo_model->get_no_po_bysupplier($kd_supplier);
	...
	public function get_all_jenis_pembayaran(){
		$result = $this->pumpo_model->get_all_jenis_pembayaran(true);
