...
class Pembelian_create_po_non_request extends MY_Controller {
	/**
	 * @author dhamarsu
	 * @editedby luxse
    ...
    public function get_form(){
    	// $no_po = 'PN' . date('Ymd') . '-';
    	// $sequence = $this->pcpnr_model->get_kode_sequence($no_po, 3);
    ...
    public function update_row(){
		...
		$header_result = $this->pcpnr_model->insert_row('purchase.t_purchase', $header_po);
		...
		foreach($detail as $obj){
			...
			if($this->pcpnr_model->insert_row('purchase.t_purchase_detail', $detail_pr)){
				$detail_result++;
			}
		...
		if ($header_result && $detail_result > 0) {
			...
			$result = '{"success":true,"errMsg":"Pembuatan PO Non PR berhasil, NO PN: '.$no_po.' ","printUrl":"' . site_url("pembelian_create_po_non_request/print_form/" . $no_po . $sequence) . '"}';
		} else {
			$this->db->trans_rollback();
	...
	public function search_produk_by_supplier(){			
		...
		//$kd_peruntukkan = isset($_POST['kd_peruntukan']) ? $this->db->escape_str($this->input->post('kd_peruntukan',TRUE)) : '';			
		...
		$result = $this->pcpnr_model->search_produk_by_supplier($kd_supplier,$sender,$waktu_top, $search);
		foreach($result as $obj){
                    
                    if($kd_peruntukkan == 1){
			$obj->disk_persen_supp1_po = $obj->disk_persen_dist1;
			$obj->disk_persen_supp2_po = $obj->disk_persen_dist2;
			$obj->disk_persen_supp3_po = $obj->disk_persen_dist3;
			$obj->disk_persen_supp4_po = $obj->disk_persen_dist4;
			
			$obj->disk_amt_supp1_po = $obj->disk_amt_dist1;
			$obj->disk_amt_supp2_po = $obj->disk_amt_dist2;
			$obj->disk_amt_supp3_po = $obj->disk_amt_dist3;
			$obj->disk_amt_supp4_po = $obj->disk_amt_dist4;
			$obj->disk_amt_supp5_po = $obj->disk_amt_dist5;
                        $obj->hrg_supplier = $obj->hrg_supplier_dist;
			...
			$obj->disk_amt_supp5_po = $obj->disk_amt_supp5;
                        $obj->hrg_supplier = $obj->hrg_supplier;
                    }
			//hitung diskon
			$diskon = 0;
							
			if(!empty($obj->disk_persen_supp1_po) && $obj->disk_persen_supp1_po > 0){
				$diskon_supp1 = ($obj->disk_persen_supp1_po * $obj->hrg_supplier) /100;
				$diskon_supp1_view = $obj->disk_persen_supp1_po." %";
			}else{
				if(!empty($obj->disk_amt_supp1_po) && $obj->disk_amt_supp1_po > 0){
					$diskon_supp1 = $obj->disk_amt_supp1_po;
					$diskon_supp1_view = "Rp ".$obj->disk_amt_supp1_po;
				}else{
					$diskon_supp1 = 0;
					$diskon_supp1_view = "0 %";
				}
			...
			if(!empty($obj->disk_persen_supp2_po) && $obj->disk_persen_supp2_po > 0){
				$diskon_supp2 = ($obj->disk_persen_supp2_po * ($obj->hrg_supplier - $diskon_supp1)) /100;
				$diskon_supp2_view = $obj->disk_persen_supp2_po." %";
			}else{
				if(!empty($obj->disk_amt_supp2_po) && $obj->disk_amt_supp2_po > 0){
					$diskon_supp2 = $obj->disk_amt_supp2_po;
					$diskon_supp2_view = "Rp ".$obj->disk_amt_supp2_po;
				}else{
					$diskon_supp2 = 0;
					$diskon_supp2_view = "0 %";
				}
			...
			if(!empty($obj->disk_persen_supp3_po) && $obj->disk_persen_supp3_po > 0){
				$diskon_supp3 = ($obj->disk_persen_supp3_po * ($obj->hrg_supplier - $diskon_supp1 - $diskon_supp2)) /100;
				$diskon_supp3_view = $obj->disk_persen_supp3_po." %";
			}else{
				if(!empty($obj->disk_amt_supp3_po) && $obj->disk_amt_supp3_po > 0){
					$diskon_supp3 = $obj->disk_amt_supp3_po;
					$diskon_supp3_view = "Rp ".$obj->disk_amt_supp3_po;
				}else{
					$diskon_supp3 = 0;
					$diskon_supp3_view = "0 %";
				}
			...
			if(!empty($obj->disk_persen_supp4_po) && $obj->disk_persen_supp4_po > 0){
				$diskon_supp4 = ($obj->disk_persen_supp4_po * ($obj->hrg_supplier - $diskon_supp1 - $diskon_supp2 - $diskon_supp3)) /100;
				$diskon_supp4_view = $obj->disk_persen_supp4_po." %";
			}else{
				if(!empty($obj->disk_amt_supp4_po) && $obj->disk_amt_supp4_po > 0){
					$diskon_supp4 = $obj->disk_amt_supp4_po;
					$diskon_supp4_view = "Rp ".$obj->disk_amt_supp4_po;
				}else{
					$diskon_supp4 = 0;
					$diskon_supp4_view = "0 %";
				}
			...
			$diskon_supp5_view = "Rp ".$obj->disk_amt_supp5_po;
		...
		$result = '{success:true,record:'.count($result).',data:'.json_encode($result).'}';
...
//			}
			...
			if($validate->sum != 0){
				$result = '{"success":false,"errMsg":"Ada Outstanding PO dengan Kode Produk '.$kd_produk.' sebanyak '.$validate->sum.'"}';
			}
		...
		if($action == 'validate_pr'){
			...
			// print_r($validate);
			if($validate['pr']->sum != 0){
				$result = '{"success":false,"errMsg":"Ada PR dengan Kode Produk '.$kd_produk.' sebanyak '.$validate->sum.'"}';
			}
...
//			}
	...
	public function print_form($no_po = ''){
		$data = $this->pcpnr_model->get_data_print($no_po);
	...
	public function get_nilai_parameter_pic(){
		$result = $this->pcpnr_model->get_nilai_parameter(PIC_PENERIMA_PO);
		return $result;
	}
	
	public function get_nilai_parameter_alamat(){
		$result = $this->pcpnr_model->get_nilai_parameter(ALAMAT_PENERIMA_PO);
		return $result; 
	}
	
	public function get_nilai_parameter_remark(){
		$result = $this->pcpnr_model->get_nilai_parameter(REMARK_PO);
		return $result; 
	}
	
	public function get_term_of_payment_by_supplier($kd_supplier = ''){
		$result = $this->pcpnr_model->get_term_of_payment_by_supplier($kd_supplier);
