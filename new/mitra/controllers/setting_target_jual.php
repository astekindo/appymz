...
class Setting_target_jual extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('setting_target_jual_model', 'stj_model');
    }
    ...
    public function update_row() {
        ...
        $no = 'STJ' . date('Ym') . '-';
        $no_bukti = $this->stj_model->get_kode_sequence($no, 4);
        $kd_stb = $no . $no_bukti;
        ...
        for ($i = 0; $i < count($detail); $i++) {
            if ($detail[$i]['edited'] == 'Y') {
                ...
                $old_data = $this->stj_model->get_row($bulan,$tahun,$detail[$i]['kd_kategori1'], $detail[$i]['kd_kategori2'], $detail[$i]['kd_kategori3'], $detail[$i]['kd_kategori4']);
                if ($old_data != NULL || $old_data != '') {
                    ...
                    $this->stj_model->update_row( $update_data,$bulan,$tahun,$detail[$i]['kd_kategori1'], $detail[$i]['kd_kategori2'], $detail[$i]['kd_kategori3'], $detail[$i]['kd_kategori4']);
                    ...
                    $this->stj_model->insert_row($new_data);
               }
                $count++;
            }
        }
    ...
    public function search_kategori() {
        ...
        $hasil = $this->stj_model->search_kategori($bulan,$tahun,$kd_kategori1, $kd_kategori2, $kd_kategori3, $kd_kategori4, $search);
        //$results = array();
