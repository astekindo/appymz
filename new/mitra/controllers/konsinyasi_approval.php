...
class Konsinyasi_approval extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
		$this->load->model('konsinyasi_approval_model');
    }
	...
	public function get_rows(){
        ...
        $result = $this->konsinyasi_approval_model->get_rows($search, $start, $limit);
        
        echo $result;
	}
	
	public function get_rows_detail($no_ro=''){
        $result = $this->konsinyasi_approval_model->get_rows_detail($no_ro);
	...
	public function update_row(){
	    ...
	    $this->db->trans_begin();
		if ($this->konsinyasi_approval_model->update_row($no_ro, $datau)) {
		...
		foreach($detail as $obj){
			...
			if ( $obj->qty != $obj->qty_adj && $keterangan == ''){
				$result = '{"success":false,"errMsg":"Keterangan Harus Diisi"}';
				$this->db->trans_rollback();
				// exit;
			}else{
				if ($this->konsinyasi_approval_model->update_row_detail($no_ro_det, $kd_prod_det, $detail_app)) {
					$result = '{"success":true,"errMsg":""}';
	...
	public function update_row_detail(){
	    ...
	    if ($qty_adj != $qty && $keterangan == ''){
			...
			if ($this->konsinyasi_approval_model->update_row_detail($no_ro_det, $kd_prod_det, $datau)) {
				$result = '{"success":true,"errMsg":""}';
				$this->db->trans_commit();
			} else {
				$result = '{"success":false,"errMsg":"Process Failed.."}';
