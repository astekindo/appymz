...
class Pembelian_receive_order_bonus extends MY_Controller {
	/**
	 * @author dhamarsu
	 * @editedby luxse
    ...
    public function __construct() {
        parent::__construct();
		$this->load->model('pembelian_receive_order_bonus_model', 'prob_model');
    }
    ...
    public function get_form(){
    	// $no_do = 'RB' . date('Ymd') . '-';
    	// $sequence = $this->prob_model->get_kode_sequence($no_do, 3);
    	echo '{"success":true,
				"data":{
					"no_do":"",
					"tanggal":"' . date('d-M-Y'). '",
    ...
    public function update_row(){
                ...
                $no_do = 'RB' . $tanggal_terima . '-';
                $sequence = $this->prob_model->get_kode_sequence($no_do, 3);
                $no_do = $no_do . $sequence;
                
		$no_po_induk = isset($_POST['no_po_induk']) ? $this->db->escape_str($this->input->post('no_po_induk',TRUE)) : '';
		...
		if(count($detail) > 0){	
			...
			$tgl_ro = strtotime($tanggal_terima);
                        foreach ($detail as $obj) {
                           $tgl_po = strtotime($obj->tanggal_po);  
                           if($tgl_ro < $tgl_po){
			...
			$header_do['no_po_induk'] = $no_po_induk;
                        $header_do['kd_peruntukan'] = $kd_peruntukan;
			 
			$header_result = $this->prob_model->insert_row('purchase.t_receive_order', $header_do);
			...
			foreach($detail as $obj){
				...
				if($this->prob_model->insert_row('purchase.t_dtl_receive_order', $detail_do)){
					$detail_result++;
					unset($trx_inventory);
					...
					$this->prob_model->insert_row('inv.t_trx_inventory', $trx_inventory);
					
                                        if($this->prob_model->get_stok_inventory($obj->kd_produk,$kd_lokasi,$kd_blok,$kd_sub_blok)){
						...
						$this->prob_model->query_update($sql);
					}
					else{
						unset($brg_inventory);
						...
						$this->prob_model->insert_row("inv.t_brg_inventory",$brg_inventory);
					}	
				}
			}
			$this->db->trans_complete();
		}
		$title = 'RECEIVE ORDER BONUS';
		if ($header_result && $detail_result > 0) {		
			$result = '{"success":true,"errMsg":"Pembuatan RO Bonus berhasil, NO RB: '.$no_do.' ","printUrl":"' . site_url("pembelian_receive_order/print_form/" . $no_do . "/" .$title) . '"}';
		} else {
	...
	public function get_all_po(){
		$kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier',TRUE)) : "";
                $no_po_induk = isset($_POST['no_po_induk']) ? $this->db->escape_str($this->input->post('no_po_induk',TRUE)) : "";
		$result = $this->prob_model->get_all_po($kd_supplier,$no_po_induk);
		...
		$kd_peruntukan_dist = isset($_POST['kd_peruntukan_dist']) ? $this->db->escape_str($this->input->post('kd_peruntukan_dist', TRUE)) : '';
                ...
                $result = $this->prob_model->get_rows_lokasi($kd_produk,$kd_peruntukan_supp,$kd_peruntukan_dist, $search, $start, $limit);
	...
	public function get_po_detail(){
		...
		$result = $this->prob_model->get_po_detail($no_po);
	...
	public function search_supplier(){			
		...
		$result = $this->prob_model->search_supplier($search, $start, $limit);
	...
	public function search_produk_by_no_po(){
		...
		$result = $this->prob_model->get_po_detail($no_po, $search);
