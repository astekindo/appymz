...
//		$limit = isset($_POST['limit']) ? $this->db->escape_str($this->input->post('limit',TRUE)) : $this->config->item("length_records");
    ...
    public function get_rows_jenisvoucher_akun(){
        $kd_jenis_voucher = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query', TRUE)) : '';
        $result = $this->transaksi_model->get_rows_jenisvoucher_akun($kd_jenis_voucher);
        echo $result;
    }
                ...
                if (count($data_akun) > 0) {
                    foreach ($data_akun as $obj) {
                        ...
                        $data_detail = array(
                            ...
                            'kd_costcenter' => $obj->kd_costcenter
                        );
...
//            $updated_date = date('Y-m-d H:i:s');
    ...
    public function get_akun_costcenter() {
        $kdcc=isset($_POST['kdcc']) ? $this->db->escape_str($this->input->post('kdcc', TRUE)) : FALSE;
        $kdakun=isset($_POST['kdakun']) ? $this->db->escape_str($this->input->post('kdakun', TRUE)) : FALSE;
        $result = $this->transaksi_model->cek_exists_costcenter($kdcc,$kdakun);
