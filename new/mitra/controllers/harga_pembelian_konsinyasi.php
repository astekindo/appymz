...
class Harga_pembelian_konsinyasi extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('harga_pembelian_konsinyasi_model', 'hpk_model');
    }
    ...
    public function get_form() {
        $no_hp = 'HP' . date('Ymd') . '-';
        $sequence = $this->hpk_model->get_kode_sequence($no_hp, 3);
        echo '{"success":true,
				"data":{
    ...
    public function update_row() {
        ...
        //$tgl_start_diskon = isset($_POST['tgl_start']) ? $this->db->escape_str($this->input->post('tgl_start', TRUE)) : '';
        //$tgl_end_diskon = isset($_POST['tgl_end']) ? $this->db->escape_str($this->input->post('tgl_end', TRUE)) : '';

        $current_date = date('Ymd', strtotime($tanggal));
        $no_ret = 'HP' . $current_date . '-';
        $sequence = $this->hpk_model->get_kode_sequence($no_ret, 3);
        ...
        foreach ($detail as $obj) {
            if ($obj->edited == 'Y') {
                ...
                $detail_hp['konsinyasi'] = '1';
                $detail_hp['tgl_start_diskon'] = $tgl_start_diskon;
                $detail_hp['aktif_diskon'] = 1;

//                if ($no_bukti_filter != '') {//No Bukti Ada
//                        unset($detail_hp['no_bukti']);
//                        $detail_hp['status'] = 0;
//                        if ($this->hpk_model->update_temp($kd_supplier, $kd_produk, $no_bukti_filter,$detail_hp)) {
//                            $results = 'success';
//                        } else {
...
//                        }
//                   
//                } else {//No Bukti Tidak Ada                 
                            $result_data = $this->hpk_model->select_data_temp($kd_produk, $tgl_start_diskon);
                                if ($result_data) {
                                    $detail_hp['status'] = 0;
                                    if ($this->hpk_model->update_temp_by_date($kd_produk, $tgl_start_diskon,$detail_hp)) {
                                        $results = 'success';
                                    }else {
                                        $this->db->trans_rollback();
                                        echo '{"success":false,"errMsg":"Update_temp Failed . . ."}';
                                    ...
                                    if ($this->hpk_model->insert_temp($detail_hp)) {
                                        $results = 'success';
                                    } else {
                                        $this->db->trans_rollback();
        ...
        if ($results == 'success') {
            ...
            $result = '{"success":true,"errMsg":"Data berhasil disimpan"}';
        } else {
    ...
    public function update_row_dist() {
        ...
        foreach ($detail as $obj) {
            if ($obj->edited == 'Y') {
                ...
                if ($no_bukti_filter != '') {
                    unset($detail_hp['no_bukti']);
                    if ($this->hpk_model->update_temp($kd_supplier, $kd_produk, $waktu_top, $no_bukti_filter, $detail_hp)) {
                        $results = 'success';
                    } else {
                        $this->db->trans_rollback();
                        echo '{"success":false,"errMsg":"update_temp Failed . . ."}';
                    ...
                    if ($this->hpk_model->select_temp($kd_supplier, $kd_produk, '0')) {
                        $this->db->trans_rollback();
                        echo '{"success":false,"errMsg":"Barang dengan Kode Barang: ' . $kd_produk . ' Belum Diapprove"}';
                        exit;
                        // }else if($this->hpk_model->select_temp($kd_supplier,$kd_produk,'1')){
                    } else {
                        $detail_hp['kd_supplier'] = $kd_supplier;
                        $detail_hp['kd_produk'] = $kd_produk;
                        $detail_hp['waktu_top'] = $waktu_top;
    ...
    public function search_supplier() {
        ...
        $result = $this->hpk_model->search_supplier($search, $start, $limit);
    ...
    public function get_nobukti() {
        ...
        $result = $this->hpk_model->get_nobukti($search, $start, $limit);

        echo $result;
    }

    public function get_nobukti_all($no_bukti = '') {
        $result = $this->hpk_model->get_nobukti_all($no_bukti);
        echo $result;
    }
    ...
    public function search_no_bukti() {
        ...
        $result = $this->hpk_model->search_no_bukti($search, $start, $limit);
    ...
    public function search_no_bukti_dist() {
        ...
        $result = $this->hpk_model->search_no_bukti_dist($search, $start, $limit);
    ...
    public function search_produk_by_supplier() {
        ...
        $tgl_start = isset($_POST['tgl_start']) ? $this->db->escape_str($this->input->post('tgl_start', TRUE)) : '';
        $tgl_end = isset($_POST['tgl_end']) ? $this->db->escape_str($this->input->post('tgl_end', TRUE)) : '';
        ...
        $data_result = $this->hpk_model->search_produk_by_supplier($kd_supplier, $no_bukti, $kd_kategori1, $kd_kategori2, $kd_kategori3, $kd_kategori4, $kd_ukuran, $kd_satuan, $tgl_start, $tgl_end, $list, $search, $start, $limit);
    ...
    public function search_produk_history() {
        ...
        $hasil = $this->hpk_model->search_produk_history($no_bukti, $kd_produk);
        $results = array();
        foreach ($hasil as $result) {
            //hitung diskon
            $diskon = 0;
    ...
    public function search_produk_history_dist() {
        ...
        $hasil = $this->hpk_model->search_produk_history_dist($no_bukti, $kd_produk);
        $results = array();
        foreach ($hasil as $result) {
            //hitung diskon
            $diskon = 0;
    ...
    public function search_kd_produk($search_by) {
        ...
        echo $this->hpk_model->get_produk($keyword);
    }

    public function print_form($no_bukti = '', $kd_produk = '') {
        $data = $this->hpk_model->get_data_print($no_bukti, $kd_produk);
        if (!$data)
            show_404('page');
    ...
    public function get_no_bukti_filter() {
        ...
        $result = $this->hpk_model->get_no_bukti_filter($kd_supplier,$search, $start, $limit);
    ...
    public function get_no_bukti_filter_dist() {
        ...
        $result = $this->hpk_model->get_no_bukti_filter_dist($search, $start, $limit);
