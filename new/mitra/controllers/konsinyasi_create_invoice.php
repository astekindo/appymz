...
//                                        if($obj->adjust != '0'){
//						if($this->kci_model->update_row_hpp($obj->adjust,$obj->kd_produk,$kd_peruntukan,$no_invoice)){
//							$no_bukti = "MB".date('Ym').'-'.$this->pci_model->get_kode_sequence("MB".date('Ym'),5);
//							if($this->kci_model->insert_row_histo($no_bukti)){
//								$detail_result++;
//							}
//						}
//						$cogs_result = $this->kci_model->get_cogs_on_hpp($obj->kd_produk);
//						
//						$dataUpdate = array(
...
//						// print_r($dataUpdate);
//						// exit;
//						if($this->kci_model->update_row_produk($obj->kd_produk,$dataUpdate)){
//							$detail_result++;
//						}else
//							$detail_result = 0;
//					}
...
//						$this->kci_model->query_update($sql);
					}
//                                        unset($data_receive);
//                                        $data_receive['is_invoice'] = 1; 
//                                        $this->kci_model->update_receive_order($obj->no_do, $data_receive);
				// }
			}
		...
		if ($header_result && $detail_result > 0) {
			$result = '{"success":true,"errMsg":"","printUrl":"' . site_url("konsinyasi_create_invoice/print_form/" . $no_invoice) . '"}';
		} else {
    ...
    public function print_form($no_inv){
		$data = $this->kci_model->get_data_print($no_inv);
		if(!$data) show_404('page');
				
		$this->output->set_content_type("application/pdf");
		require_once(APPPATH . 'libraries/KonsinyasiCreateInvoicePrint.php');
		$pdf = new KonsinyasiCreateInvoicePrint(PDF_PAGE_ORIENTATION_LANDSCAPE, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$pdf->setKertas();
	...
	public function search_no_do_by_supplier(){
		...
		$no_do = isset($_POST['no_do']) ? $this->db->escape_str($this->input->post('no_do',TRUE)) : '';
                $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query',TRUE)) : '';
		$result = $this->kci_model->search_no_do_by_supplier($kd_supplier, $no_do, $search);
