...
class Harga_pembelian extends MY_Controller {
    ...
    public function update_row(){
		...
		$keterangan = isset($_POST['keterangan']) ? $this->db->escape_str($this->input->post('keterangan',TRUE)) : '';
                ...
                $no_ret = 'HP' . $current_date . '-';
                $sequence = $this->hp_model->get_kode_sequence($no_ret, 3);
                $no_hp = "$no_ret$sequence";
		...
		foreach($detail as $obj){
			if($obj->edited == 'Y'){
				...
				$aktif = '1';
                                $tgl_start = $obj->tgl_start_diskon;
                                if($tgl_start){
                                    $tgl_start_diskon = date('Y-m-d', strtotime($tgl_start));
                                }
				...
				$detail_hp['no_bukti']	=	$no_hp;
                                $detail_hp['konsinyasi'] = '0';
                                $detail_hp['tgl_start_diskon'] = $tgl_start_diskon;
                                $detail_hp['aktif_diskon'] = 1;
                                
                                $result_data = $this->hp_model->select_data_temp($kd_produk, $tgl_start_diskon);
                                if ($result_data) {
                                    $detail_hp['status'] = 0;
                                    if ($this->hp_model->update_temp_by_date($kd_produk, $tgl_start_diskon,$detail_hp)) {
                                        $results = 'success';
                                    }else {
...
public function update_row_dist(){
		$no_hp = isset($_POST['no_hp']) ? $this->db->escape_str($this->input->post('no_hp',TRUE)) : '';
		$no_bukti_filter = isset($_POST['no_bukti_filter']) ? $this->db->escape_str($this->input->post('no_bukti_filter',TRUE)) : '';
		$kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier',TRUE)) : '';
		$tanggal = isset($_POST['tanggal']) ? $this->db->escape_str($this->input->post('tanggal',TRUE)) : '';
	...
	public function search_no_bukti(){
        ...
        public function search_no_bukti_dist(){
		$start = isset($_POST['start']) ? $this->db->escape_str($this->input->post('start',TRUE)) : 0;
		$limit = isset($_POST['limit']) ? $this->db->escape_str($this->input->post('limit',TRUE)) : $this->config->item("length_records");
        $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query',TRUE)) : '';

		$result = $this->hp_model->search_no_bukti_dist($search, $start, $limit);
	...
	public function search_produk_by_supplier(){
		...
		foreach($hasil as $result) {
			$results[] = $this->kalkulasiDiskonPembelian($result);
		}
	...
	public function search_produk_history(){
        ...
        public function search_produk_history_dist(){
		$no_bukti = isset($_POST['no_bukti']) ? $this->db->escape_str($this->input->post('no_bukti',TRUE)) : '';
		$kd_produk = isset($_POST['kd_produk']) ? $this->db->escape_str($this->input->post('kd_produk',TRUE)) : '';

		$hasil = $this->hp_model->search_produk_history_dist($no_bukti,$kd_produk);
		$results = array();
		foreach($hasil as $result){
			$this->kalkulasiDiskonPembelian($result);
			$results[] = $result;
		}
		echo '{success:true,data:'.json_encode($results).'}';
	}
	...
	public function print_form($no_bukti = '', $kd_produk = ''){
        ...
        public function print_form_dist($no_bukti = '', $kd_produk = ''){
		$data = $this->hp_model->get_data_print($no_bukti,$kd_produk);
		if(!$data) show_404('page');

		$this->output->set_content_type("application/pdf");
		require_once(APPPATH . 'libraries/HargaPembelianDistPrint.php');
		$pdf = new HargaPembelianDistPrint(PDF_PAGE_ORIENTATION_LANDSCAPE, PDF_UNIT, 'F4', true, 'UTF-8', false);
		$pdf->setKertas();
		$pdf->privateData($data['detail']);
		$pdf->Output();
		exit;
	...
	public function get_no_bukti_filter(){
		...
		$result = $this->hp_model->get_no_bukti_filter_dist($search, $start, $limit);

        echo $result;
	}

	protected function kalkulasiDiskonPembelian($produk) {
        //hitung diskon
        $diskon = 0;

        if(!empty($produk->disk_persen_supp1)){
            $diskon_supp1 = $produk->disk_persen_supp1;
            $produk->disk_supp1_op = "%";
        }elseif(!empty($produk->disk_amt_supp1)){
                $diskon_supp1 = $produk->disk_amt_supp1;
                $produk->disk_supp1_op = "Rp";
        }else{
            $diskon_supp1 = 0;
        }

        if(!empty($produk->disk_persen_supp2)){
            $diskon_supp2 = $produk->disk_persen_supp2;
            $produk->disk_supp2_op = "%";
        }elseif(!empty($produk->disk_amt_supp2)){
                $diskon_supp2 = $produk->disk_amt_supp2;
                $produk->disk_supp2_op = "Rp";
        }else{
            $diskon_supp2 = 0;
        }

        if(!empty($produk->disk_persen_supp3)){
            $diskon_supp3 = $produk->disk_persen_supp3;
            $produk->disk_supp3_op = "%";
        }elseif(!empty($produk->disk_amt_supp3)){
                $diskon_supp3 = $produk->disk_amt_supp3;
                $produk->disk_supp3_op = "Rp";
        }else{
            $diskon_supp3 = 0;
        }

        if(!empty($produk->disk_persen_supp4)){
            $diskon_supp4 = $produk->disk_persen_supp4;
            $produk->disk_supp4_op = "%";
        }elseif(!empty($produk->disk_amt_supp4)){
                $diskon_supp4 = $produk->disk_amt_supp4;
                $produk->disk_supp4_op = "Rp";
        }else{
            $diskon_supp4 = 0;
        }

        if(!empty($produk->diskon_amt_supp5)){
            $diskon_amt_supp5 = $produk->diskon_amt_supp5;
        }else{
            $diskon_amt_supp5 = 0;
        }
        ...
        //diskon Rp
        $produk->disk_supp1 = $diskon_supp1;
        $produk->disk_supp2 = $diskon_supp2;
        $produk->disk_supp3 = $diskon_supp3;
        $produk->disk_supp4 = $diskon_supp4;

        $diskon = 0;

        if(!empty($produk->disk_persen_dist1) && $produk->disk_persen_dist1 != 0){
            $diskon_dist1 = $produk->disk_persen_dist1;
            $produk->disk_dist1_op = "%";
        }elseif(!empty($produk->disk_amt_dist1)){
                $diskon_dist1 = $produk->disk_amt_dist1;
                $produk->disk_dist1_op = "Rp";
        }else{
            $diskon_dist1 = 0;
        }

        if(!empty($produk->disk_persen_dist2) && $produk->disk_persen_dist2 != 0){
            $diskon_dist2 = $produk->disk_persen_dist2;
            $produk->disk_dist2_op = "%";
        }elseif(!empty($produk->disk_amt_dist2)){
                $diskon_dist2 = $produk->disk_amt_dist2;
                $produk->disk_dist2_op = "Rp";
        }else{
            $diskon_dist2 = 0;
        }

        if(!empty($produk->disk_persen_dist3) && $produk->disk_persen_dist3 != 0){
            $diskon_dist3 = $produk->disk_persen_dist3;
            $produk->disk_dist3_op = "%";
        }elseif(!empty($produk->disk_amt_dist3)){
                $diskon_dist3 = $produk->disk_amt_dist3;
                $produk->disk_dist3_op = "Rp";
        }else{
            $diskon_dist3 = 0;
        }

        if(!empty($produk->disk_persen_dist4) && $produk->disk_persen_dist4 != 0){
            $diskon_dist4 = $produk->disk_persen_dist4;
            $produk->disk_dist4_op = "%";
        }elseif(!empty($produk->disk_amt_dist4)){
                $diskon_dist4 = $produk->disk_amt_dist4;
                $produk->disk_dist4_op = "Rp";
        }else{
            $diskon_dist4 = 0;
        }

        if(!empty($produk->diskon_amt_dist5)){
            $diskon_amt_dist5 = $produk->diskon_amt_dist5;
        }else{
            $diskon_amt_dist5 = 0;
        }
        ...
        //diskon Rp
        $produk->disk_dist1 = $diskon_dist1;
        $produk->disk_dist2 = $diskon_dist2;
        $produk->disk_dist3 = $diskon_dist3;
        $produk->disk_dist4 = $diskon_dist4;
        return $produk;
	}
