...
class Harga_penjualan_proyek extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('harga_penjualan_proyek_model', 'hjp_model');
        $this->load->model('harga_penjualan_model', 'hj_model');
    }
    public function get_form() {
        $no_hjb = 'HJP' . date('Ymd') . '-';
        $sequence = $this->hjp_model->get_kode_sequence($no_hjb, 3);
        echo '{"success":true,
				"data":{
     ...
     public function search_no_bukti() {
        ...
        $result = $this->hjp_model->search_no_bukti($kd_supplier,$search, $start, $limit);
    ...
    public function search_no_bukti_filter() {
        ...
        $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query', TRUE)) : '';
        $kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier', TRUE)) : '';
        $result = $this->hjp_model->search_no_bukti_filter($kd_supplier,$search, $start, $limit);
     ...
     public function search_produk_by_kategori() {
        ...
        $data_result = $this->hjp_model->search_produk_by_kategori($kd_supplier, $no_bukti, $konsinyasi, $kd_kategori1, $kd_kategori2, $kd_kategori3, $kd_kategori4, $kd_ukuran, $kd_satuan, $list, $search, $start, $limit);
    ...
    public function update_row() {
        ...
        $no_hj = 'HJP' . date('Ymd') . '-';
        $sequence = $this->hjp_model->get_kode_sequence($no_hj, 3);
        $no_hjp = $no_hj . $sequence;
        ...
        foreach ($detail as $obj) {
            if ($obj->edited == 'Y') {
                ...
                if ($tgl_end_diskon < $tgl_now) {
                    echo '{"success":false,"errMsg":"Tanggal Akhir Diskon Tidak Boleh Lebih Kecil Dari Tanggal Hari ini "}';
                ...
                $diskon_hj['kd_diskon_sales'] = $no_hjp;
                ...
                if ($no_bukti_filter != '') {
                    ...
                    if ($this->hjp_model->update_temp($kd_produk, $no_bukti_filter, $diskon_hj)) {
                        $results = 'success';
                    } else {
                        $this->db->trans_rollback();
                    ...
                    $result = $this->hjp_model->select_temp($kd_produk, '0');
                    if (!empty($result)) {
                        $this->db->trans_rollback();
                        ...
                        // }else if($this->hjd_model->select_temp($kd_produk,'1')){
                         ...
                         $produk_sama = $this->hjp_model->select_data_proyek_sama($kd_produk, $tgl_start_diskon,$tgl_end_diskon);
                        if (!empty($produk_sama)){
                            $diskon_hj['kd_produk'] = $kd_produk;
                            $diskon_hj['status_approval'] = 0;
                            if ($this->hjp_model->insert_temp($diskon_hj)) {
                                $results = 'success';
                            } else {
                                $this->db->trans_rollback();
                            ...
                            $result_prod = $this->hjp_model->select_data_temp($kd_produk, $tgl_start_diskon,$tgl_end_diskon);
                            if (!empty($result_prod)) {
                                $this->db->trans_rollback();
                                echo '{"success":false,"errMsg":"Tanggal Awal Diskon udah ada dalam range : ' . $kd_produk . ' "}';
                                exit;
                            }else{
                                $result_data = $this->hjp_model->select_data_temp_end($kd_produk, $tgl_start_diskon,$tgl_end_diskon);
  ...
  public function search_produk_history() {
        ...
        $hasil = $this->hjp_model->search_produk_history($no_bukti, $kd_produk);
        $results = array();
        foreach ($hasil as $result) {
            //hitung diskon
            $diskon = 0;
     ...
     public function print_form($no_bukti = '', $kd_produk = '') {
        $data = $this->hjp_model->get_data_print($no_bukti, $kd_produk);
        if (!$data)
            show_404('page');

        $this->output->set_content_type("application/pdf");
        require_once(APPPATH . 'libraries/HargaPenjualanProyekPrint.php');
        $pdf = new HargaPenjualanProyekPrint(PDF_PAGE_ORIENTATION_LANDSCAPE, PDF_UNIT, 'F4', true, 'UTF-8', false);
        $pdf->setKertas();
