...
//            $arrrec = $this->apvr_acc_model->get_rows_akun_loop($v->kd_voucher);
        ...
        foreach ($arrrec as $obj) {
            $fak = 1;
            $obj->dk_akun=$this->getDKakun($obj->kd_akun);
            ...
            $djurnal['kd_costcenter'] = $obj->costcenter;
...
//                $bbsaldo['saldo'] = $saldobb;
    ...
    public function getDKakun($kdakun) {
        $retval = $this->evr_acc_model->get_dk_akun($kdakun);
        return $retval;
    }
...
//        echo "<script>console.log('".$header_do['approval1'].")</script>";
        ...
        if($header_do['diterima_oleh']){
            $header_do['diterima_oleh']=  strtoupper($header_do['diterima_oleh']);            
        }
        if($header_do['no_giro_cheque']){
            $header_do['no_giro_cheque']=strtoupper($header_do['no_giro_cheque']);
        }
...
//            }
            ...
            if ($header_result && $detail_result > 0) {
                ...
                $result = '{"success":true,"errMsg":" No.Voucher : '.$header_do['kd_voucher'].'"}';
