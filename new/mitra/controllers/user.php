...
class User extends MY_Controller {
    ...
    public function update_row() {
        ...
        $kd_cabang = isset($_POST['pkd_cabang']) ? $this->db->escape_str($this->input->post('pkd_cabang', TRUE)) : FALSE;
        $bazar = isset($_POST['rb-bazar']) ? $this->db->escape_str($this->input->post('rb-bazar', TRUE)) : FALSE;
	    ...
	    $data = array(
            ...
            //print_r($datai);
            $user = $this->user_model->search_user($username);
            if ($user){
                $result = '{"success":false,"errMsg":"User Sudah Ada"}';
                
            }
            else {
            if ($this->user_model->insert_row($data) && $this->user_model->insert_row_info($datai)) {
                $result = '{"success":true,"errMsg":"Sukses Data Disimpan"}';
            } else {
