...
class Monitoring_purchase_order extends MY_Controller {
		...
		if(!empty($d))
		{
			...
			foreach($d as $v)
			{
				...
				$detail .= '<tr>
								...
								<td align="left" bgcolor="#f5f5f5">&nbsp;&nbsp;'.$v->no_ro .'</td>
			  ...
			  <tr>
				...
				<td width="370" valign="middle"><table width="370" border="0" cellspacing="1" cellpadding="5">
				  <tr>
					...
					<td width="100" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_jumlah_po / $h->rp_diskon_po, 0,',','.').' %</td>
					<td width="130" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_diskon_po, 0,',','.').'</td>
					</tr>
				  <tr>
					<td height = "28" align="right" valign="middle" bgcolor="#ACD9EB"><b>Total Tagihan</b>&nbsp;&nbsp;</td>
					<td colspan="2" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_jumlah_po - $h->rp_diskon_po, 0,',','.').'</td>
					</tr>
				  <tr>
					<td height = "28" align="right" valign="middle" bgcolor="#ACD9EB"><b>PPN</b>&nbsp;&nbsp;</td>
					<td valign="middle" align="right" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->ppn_percent_po, 0,',','.').' %</td>
					<td valign="middle" align="right" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_ppn_po, 0,',','.').'</td>
					</tr>
				  <tr>
					<td height = "28" align="right" valign="middle" bgcolor="#ACD9EB"><b>Grand Total</b>&nbsp;&nbsp;</td>
					<td colspan="2" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_total_po, 0,',','.').'</td>
					</tr>
				  <tr>
					<td height = "28" align="right" valign="middle" bgcolor="#ACD9EB"><b>DP</b>&nbsp;&nbsp;</td>
					<td colspan="2" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_dp, 0,',','.').'</td>
					</tr>
				  <tr>
					<td height = "28" align="right" valign="middle" bgcolor="#ACD9EB"><b>Sisa Bayar</b>&nbsp;&nbsp;</td>
					<td colspan="2" align="right" valign="middle" bgcolor="#f5f5f5">&nbsp;&nbsp;'.number_format($h->rp_total_po - $h->rp_dp, 0,',','.').'</td>
					</tr>
				</table></td>
			  </tr>
			</table>
	...
	public function get_rows(){
        ...
        $close_po       = isset($_POST['close_po']) ? $this->db->escape_str($this->input->post('close_po',TRUE)) : '';
	...
	if($peruntukan_sup == 'true'){
        ...
        $result = $this->monitoring_po_model->get_rows($kdSupplier, $tglAwal, $tglAkhir, $approval_po, $close_po, $konsinyasi,$peruntukan_sup,$peruntukan_dist, $search, $start, $limit);
