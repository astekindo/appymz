...
class Konsinyasi_create_po extends MY_Controller {
	...
	public function get_nilai_parameter_remark(){
        ...
        public function search_generate_sales() {
        $start = isset($_POST['start']) ? $this->db->escape_str($this->input->post('start', TRUE)) : 0;
        $limit = isset($_POST['limit']) ? $this->db->escape_str($this->input->post('limit', TRUE)) : $this->config->item("length_records");
        $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query', TRUE)) : '';
        $kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier', TRUE)) : '';

        $result = $this->kcp_model->search_generate_sales($kd_supplier,$search, $start, $limit);

        echo $result;
    }
    public function get_data_generate_detail(){
		$no_bukti = isset($_POST['no_bukti']) ? $this->db->escape_str($this->input->post('no_bukti', TRUE)) : '';
		$detail = $this->kcp_model->get_data_generate_detail($no_bukti);
		
		$data = array();
		foreach($detail as $obj){
						
				$obj->hrg_supplier = $obj->rp_jual;
				...
				if($obj->disk_persen_supp1 != '' && $obj->disk_persen_supp1 != 0){
					$diskon_supp1 = ($obj->disk_persen_supp1 * $obj->rp_jual) /100;
					$disk_grid_supp1 = $obj->disk_persen_supp1.'%';
				}else{
				...
				if($obj->disk_persen_supp2 != '' && $obj->disk_persen_supp2 != 0){
					$diskon_supp2 = ($obj->disk_persen_supp2 * ($obj->rp_jual - $diskon_supp1)) /100;
					$disk_grid_supp2 = $obj->disk_persen_supp2.'%';
				}else{
				...
				if($obj->disk_persen_supp3 != '' && $obj->disk_persen_supp3 != 0){
					$diskon_supp3 = ($obj->disk_persen_supp3 * ($obj->rp_jual - $diskon_supp1 - $diskon_supp2)) /100;
					$disk_grid_supp3 = $obj->disk_persen_supp3.'%';
				}else{
				...
				if($obj->disk_persen_supp4 != '' && $obj->disk_persen_supp4 != 0){
					$diskon_supp4 = ($obj->disk_persen_supp4 * ($obj->rp_jual - $diskon_supp1 - $diskon_supp2 - $diskon_supp3)) /100;
					$disk_grid_supp4 = $obj->disk_persen_supp4.'%';
				}else{
				...
				$obj->disk_persen_supp5 = $diskon_supp5;
				$obj->total_diskon = $diskon;
				// $obj->total_diskon = $obj->hrg_supplier-$obj->net_hrg_supplier_sup_inc;
				...
				if($obj->pkp == 1){
					$obj->dpp_po = $obj->net_price/1.1; 
				}else{
					$obj->dpp_po = $obj->net_price;
				}
				//hitung jumlah
				$obj->jumlah = $obj->qty * $obj->dpp_po;			
				$data[] = $obj;
      ...
      public function update_row_po(){
		...
		foreach($detail as $obj){
			...
			$detail_pr['disk_amt_supp4_po'] = (int) $obj->disk_amt_supp4_po;
			$detail_pr['disk_amt_supp5_po'] = (int) $obj->disk_amt_supp5_po;
			$detail_pr['price_supp_po'] = (int) $obj->rp_jual;
			$detail_pr['net_price_po'] = (int) $obj->net_price;
			$detail_pr['dpp_po'] = (int) $obj->dpp_po;
			$detail_pr['rp_disk_po'] = $obj->total_diskon;
			$detail_pr['rp_total_po'] = (int) $obj->jumlah;
			$detail_pr['po_created_by'] = $this->session->userdata('username');
			...
			if($this->kcp_model->insert_row('purchase.t_purchase_detail', $detail_pr)){
				$this->kcp_model->update_gen_order_konsinyasi($no_bukti, $obj->kd_produk);
				$detail_result++;
			}
