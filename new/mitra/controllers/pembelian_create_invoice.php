...
class Pembelian_create_invoice extends MY_Controller {
    ...
    public function update_row(){
		//header invoice
                ...
                $current_date = date('Ymd', strtotime($tgl_terima_invoice));
		$no_in = 'IN' . $current_date .'-';
                $sequence = $this->pci_model->get_kode_sequence($no_in, 3);
    	
		$no_invoice = $no_in . $sequence;
                 $tgl_terima_invoice = date('Y-m-d', strtotime($tgl_terima_invoice));
		...
		$tgl_faktur_pajak = isset($_POST['tgl_faktur_pajak']) ? $this->db->escape_str($this->input->post('tgl_faktur_pajak',TRUE)) : FALSE;
                $tgl_faktur_pajak = date('Y-m-d', strtotime($tgl_faktur_pajak));
		...
		$tanggal_terima_invoice = isset($_POST['tgl_terima_invoice']) ? $this->db->escape_str($this->input->post('tgl_terima_invoice',TRUE)) : FALSE;
                $tgl_inv = strtotime($tanggal_terima_invoice);
                foreach($detail as $obj){
                    $tgl_ro = strtotime($obj->tanggal_terima);
                    if($tgl_inv < $tgl_ro){
                        echo '{"success":false,"errMsg":"Tanggal Terima Invoice Tidak Boleh Lebih Kecil dari Tanggal RO"}';
		...
		if(count($detail) > 0){	
			...
			$header_pr['tgl_invoice'] = $tanggal_invoice ;
			...
			foreach($detail as $obj){
				unset($detail_pr);
					...
					$detail_pr['rp_total_diskon'] = $obj->rp_disk_po;
					...
					$detail_pr['rp_ajd_jumlah'] = $obj->adjust;
                                        $detail_pr['harga_net'] = $obj->harga_net;
					...
					if($this->pci_model->insert_row('purchase.t_invoice_detail', $detail_pr)){
						$detail_result++;
                                                ...
                                                $sql = "UPDATE purchase.t_dtl_receive_order SET qty_invoice = qty_invoice + " . $obj->qty_terima . ",flag_invoice = 1 WHERE kd_produk = '" . $obj->kd_produk . "' AND no_do = '" . $obj->no_do . "'";
						$this->pci_model->query_update($sql);
	...
	public function search_no_do_by_supplier(){
		$kd_supplier = isset($_POST['kd_supplier']) ? $this->db->escape_str($this->input->post('kd_supplier',TRUE)) : '';
                $kd_peruntukan = isset($_POST['kd_peruntukkan']) ? $this->db->escape_str($this->input->post('kd_peruntukkan',TRUE)) : '';
		$no_do = isset($_POST['no_do']) ? $this->db->escape_str($this->input->post('no_do',TRUE)) : '';
                $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query',TRUE)) : '';
//                if($search != ''){
//			$search_exp = explode(',',$search);
//			$search_imp = implode("','",$search_exp);
//			$search = strtoupper("'".$search_imp."'");
//		}
		$result = $this->pci_model->search_no_do_by_supplier($kd_supplier, $no_do, $search, $kd_peruntukan);		
                echo $result;
	}
	...
	public function search_no_do_by_supplier_no_do(){
		...
		foreach($hasil as $result){
			//hitung diskon
			$diskon = 0;
                        $test = ($result->disk_persen_supp1_po);
			if($result->disk_persen_supp1_po != '' && $result->disk_persen_supp1_po != 0){
				...
				$disk_grid_supp1 = $result->disk_persen_supp1_po.'%';
			}else{
			...
			if($result->disk_persen_supp2_po != '' && $result->disk_persen_supp2_po != 0){
				...
				$disk_grid_supp2 = $result->disk_persen_supp2_po.'%';
			}else{
			...
			if($result->disk_persen_supp3_po != '' && $result->disk_persen_supp3_po != 0){
				...
				$disk_grid_supp3 = $result->disk_persen_supp3_po.'%';
			}else{
			...
			if($result->disk_persen_supp4_po != '' && $result->disk_persen_supp4_po != 0){
				...
				$disk_grid_supp4 = $result->disk_persen_supp4_po.'%';
			}else{
			...
			if($result->disk_amt_supp5_po != '' && $result->disk_amt_supp5_po != 0){
				$diskon_amt_supp5_po = $result->disk_amt_supp5_po;
