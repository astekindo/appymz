...
class Konsinyasi_view_invoice extends MY_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('konsinyasi_view_invoice_model');
    }
    public function get_data_invoice($no_invoice = ''){
		$data = $this->konsinyasi_view_invoice_model->get_data_html($no_invoice);
		...
		$header = '
			<table width="1000" border="0" cellspacing="1" cellpadding="5">
			  ...
			  <tr>
				...
				<td align="right" valign="middle" bgcolor="#ACD9EB"><b>No Bukti Supplier&nbsp;&nbsp;</b></td>
    ...
    public function search_noinvoice() {
        ...
        $result = $this->konsinyasi_view_invoice_model->search_noinvoice($kd_peruntukan,$search, $start, $limit);
    ...
    public function get_rows() {
       ...
       // $kd_produk = isset($_POST['kd_produk']) ? $this->db->escape_str($this->input->post('kd_produk', TRUE)) : '';
        ...
        $result = $this->konsinyasi_view_invoice_model->get_rows($peruntukan_sup,$peruntukan_dist,$tglAwal, $tglAkhir, $no_invoice, $kd_supplier, $search, $start, $limit);
