...
class In_out_stok extends MY_Controller {
    ...
    public function search_lokasi() {
        $kd_produk = isset($_POST['kd_produk']) ? $this->db->escape_str($this->input->post('kd_produk', TRUE)) : 0;
        $start = isset($_POST['start']) ? $this->db->escape_str($this->input->post('start', TRUE)) : 0;
        $limit = isset($_POST['limit']) ? $this->db->escape_str($this->input->post('limit', TRUE)) : $this->config->item("length_records");
        $search = isset($_POST['query']) ? $this->db->escape_str($this->input->post('query', TRUE)) : '';


        $result = $this->in_out_stok_model->search_lokasi($search, $start, $limit, $kd_produk);
    ...
    public function update_row() {
        ...
        foreach ($detail as $obj) {
            ...
            if ($obj->edited == 'Y') {
                ...
                if(empty($keterangan)) {
                    echo '{"success":false,"errMsg":"Keterangan mutasi tidak boleh kosong!"}';
        ...
        if ($detailresult) {
            $result = '{"success":true,"errMsg":"","printUrl":"' . site_url("in_out_stok/print_ios/" . $no_bukti) . '"}';
        } else {
...
//        var_dump($data_result);
    ...
    public function print_ios($no_bukti) {
        if(!$no_bukti) show_404('page');

        $data = array(
            'header' => $this->in_out_stok_model->get_summary_print($no_bukti),
            'detail' => $this->in_out_stok_model->get_detail_print($no_bukti),
        );

        $data['header']->title = 'Form In/Out Stok';
        $this->output->set_content_type("application/pdf");
        require_once(APPPATH . 'libraries/In_out_stok_pdf.php');
        $pdf = new In_out_stok_pdf(PDF_PAGE_ORIENTATION_LANDSCAPE, PDF_UNIT, 'LETTER_MBS', true, 'UTF-8', false);
        $pdf->setKertas();
