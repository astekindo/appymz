...
class Approve_po extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
		$this->load->model('approve_po_model');
    }
	...
	public function get_rows_detail($no_po=''){
        ...
        $hasil = $this->approve_po_model->get_rows_detail($no_po, $start, $limit);
		$results = array();
		
		foreach($hasil as $result){
			
				// $result->disk_persen_supp1_po = $obj->disk_persen_supp1_po;
				// $result->disk_persen_supp2_po = $obj->disk_persen_supp2_po;
				// $result->disk_persen_supp3_po = $obj->disk_persen_supp3_po;
				// $result->disk_persen_supp4_po = $obj->disk_persen_supp4_po;
				
				// $result->disk_amt_supp1_po = $obj->disk_amt_supp1_po;
				// $result->disk_amt_supp2_po = $obj->disk_amt_supp2_po;
				// $result->disk_amt_supp3_po = $obj->disk_amt_supp3_po;
				// $result->disk_amt_supp4_po = $obj->disk_amt_supp4_po;
	...
	public function get_rows(){
        ...
        $result = $this->approve_po_model->get_rows($kd_peruntukan,$search, $start, $limit);
	...
	public function update_row(){
		...
		$qty_result = $this->approve_po_model->get_jumlah_po($no_po);
		 //print_r($qty_result);
		 //exit;
		if($status == '9'){
                    $update_status = true;    
                    foreach ($qty_result as $result){
                        $current_status = $this->approve_po_model->update_qty_po($result->qty_po,$result->no_ro,$result->kd_produk);
                        if(!$current_status) {
                            $update_status = false;
                            echo '{"success":false,"errMsg":"Gagal update"}';
...
//                    } 
		...
		if ($this->approve_po_model->update_row($no_po, $datau)) {
			$result = '{"success":true,"errMsg":""}';
		} else {
			$result = '{"success":false,"errMsg":"Process Failed.."}';
		}
