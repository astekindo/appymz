...
class Pembelian_retur extends MY_Controller {
    ...
    public function get_all_po() {
        ...
        $result = $this->pret_model->get_all_po($no_ro, $search);
    ...
    public function search_supplier() {
        ...
        $hasil = $this->pret_model->search_supplier($search, $start, $limit);
    ...
    public function search_produk_by_supplier() {
        ...
        $hasil = $this->pret_model->search_produk_by_supplier($kd_supplier, $kd_produk,$no_ro,$no_po, $search);
        $results = array();
        foreach ($hasil as $result) {
            //hitung diskon
            $diskon = 0;
            ...
            if ($result->disk_persen_supp2 != '' && $result->disk_persen_supp2 != 0) {
                ...
                $diskon_supp_hitung = $diskon_supp_hitung - ($diskon_supp_hitung * ($result->disk_persen_supp2 / 100));
            ...
            if ($result->disk_persen_supp3 != '' && $result->disk_persen_supp3 != 0) {
                ...
                $diskon_supp_hitung = $diskon_supp_hitung - ($diskon_supp_hitung * ($result->disk_persen_supp3 / 100));
            ...
            if ($result->disk_persen_supp4 != '' && $result->disk_persen_supp4 != 0) {
                ...
                $diskon_supp_hitung = $diskon_supp_hitung - ($diskon_supp_hitung * ($result->disk_persen_supp4 / 100));
            ...
            $result->diskon_amt_supp5 = $diskon_amt_supp5;
            
            $result->rp_total_diskon = $result->hrg_supplier - $diskon_supp_hitung;
            

            $diskon = $diskon_supp1_hitung + $diskon_supp2_hitung + $diskon_supp3_hitung + $diskon_supp4_hitung + $diskon_amt_supp5;

            //hitung harga  
            $result->harga = $diskon_supp_hitung;
    ...
    public function search_produk_by_no_invoice() {
        ...
        foreach ($hasil as $result) {
            ...
            if ($result->disk_persen_supp1 != '' && $result->disk_persen_supp1 != 0) {
                $diskon_supp1 = $result->disk_persen_supp1;
                $diskon_supp_hitung = ($result->disk_persen_supp1 * $result->hrg_supplier) / 100;
                $result->disk_supp1_op = "%";
            } else {
                if ($result->disk_amt_supp1 != '') {
                    $diskon_supp1 = $result->disk_amt_supp1;
                    $result->hrg_jual_toko - $result->disk_amt1;
                    $diskon_supp_hitung = $result->hrg_supplier - $diskon_supp1;
                    $result->disk_supp1_op = "Rp";
                } else {
                    $diskon_supp1 = 0;
            ...
            //hitung diskon 
            $result->rp_total_diskon = $diskon_supp_hitung;
            
            //hitung harga  
            //$result->harga = $result->hrg_supplier - $diskon;
            $result->hrg_supplier = $result->hrg_supplier;
            $result->harga =  $result->hrg_supplier - $diskon_supp_hitung;
    ...
    public function search_produk_by_no_po() {
        ...
        $hasil = $this->pret_model->search_produk_by_no_po($kd_supplier, $kd_produk,$kd_peruntukan, $query);
        $results = array();
        foreach ($hasil as $result) {
            //hitung diskon
            $diskon = 0;
    ...
    public function update_row() {
        ...
        foreach ($detail as $obj) {
            ...
            $disk_amt_supp5 = $obj->disk_amt_supp5;
            ...
            $detail_ret['rp_diskon'] =  $obj->rp_total_diskon;
            ...
            $detail_ret['no_invoice'] = $obj->no_invoice;
            ...
            if ($this->pret_model->insert_row('purchase.t_retur_purchase_detail', $detail_ret)) {
                $sql = "UPDATE purchase.t_invoice_detail SET qty_retur = qty_retur + " . $obj->qty . " WHERE kd_produk = '" . $obj->kd_produk . "' AND no_invoice = '".$obj->no_invoice."'";
		$this->pret_model->query_update($sql);
    ...
    public function update_row_by_RO() {
        $tgl_retur = isset($_POST['tgl_retur']) ? $this->db->escape_str($this->input->post('tgl_retur', TRUE)) : FALSE;
        $current_date = date('Ym', strtotime($tgl_retur));
        $no_ret = 'RR' . $current_date . '-';
        $sequence = $this->pret_model->get_kode_sequence($no_ret, 3);
        $no_retur = $no_ret . $sequence;
        ...
        $pcn_ppn = isset($_POST['_ppn_persen']) ? $this->db->escape_str($this->input->post('_ppn_persen', TRUE)) : FALSE;
        $rp_jumlah = isset($_POST['_jumlah']) ? $this->db->escape_str($this->input->post('_jumlah', TRUE)) : FALSE;
        $no_do = isset($_POST['_no_do']) ? $this->db->escape_str($this->input->post('_no_do', TRUE)) : FALSE;
        //$no_po = isset($_POST['_no_po']) ? $this->db->escape_str($this->input->post('_no_po', TRUE)) : FALSE;
        $rp_ppn = isset($_POST['_ppn_rp']) ? $this->db->escape_str($this->input->post('_ppn_rp', TRUE)) : FALSE;
        $rp_total = isset($_POST['_total']) ? $this->db->escape_str($this->input->post('_total', TRUE)) : FALSE;
        $remark = isset($_POST['_remark']) ? $this->db->escape_str($this->input->post('_remark', TRUE)) : FALSE;
        $kd_lokasi = isset($_POST['kd_lokasi']) ? $this->db->escape_str($this->input->post('kd_lokasi', TRUE)) : FALSE;
        ...
        $header_ret['created_date'] = $created_date;
        $header_ret['remark'] = $remark;
        $header_ret['retur_type'] = '0';
        $header_ret['is_konsinyasi'] = '0';
       // print_r($header_ret);exit;
        $header_result = $this->pret_model->insert_row('purchase.t_retur_purchase', $header_ret);
        ...
        foreach ($detail as $obj) {
            ...
            $detail_ret['qty'] = (int) $obj->qty;
            $detail_ret['price_supp'] = (int) $obj->hrg_supplier;
            $detail_ret['disk_persen_supp1'] =  $disk_persen_supp1;
            $detail_ret['disk_persen_supp2'] =  $disk_persen_supp2;
            $detail_ret['disk_persen_supp3'] =  $disk_persen_supp3;
            $detail_ret['disk_persen_supp4'] =  $disk_persen_supp4;
            $detail_ret['disk_amt_supp1'] =  $disk_amt_supp1;
            $detail_ret['disk_amt_supp2'] =  $disk_amt_supp2;
            $detail_ret['disk_amt_supp3'] =  $disk_amt_supp3;
            $detail_ret['disk_amt_supp4'] =  $disk_amt_supp4;
            $detail_ret['disk_amt_supp5'] =  $disk_amt_supp5;
            $detail_ret['rp_diskon'] =  $obj->rp_total_diskon;
            $detail_ret['net_price'] = $obj->harga;
            ...
            $detail_ret['kd_blok'] = $kd_blok;
            $detail_ret['kd_sub_blok'] = $kd_sub_blok;
            $detail_ret['no_po'] = $obj->no_po;
            $detail_ret['no_do'] = $no_do;
            ...
            if ($this->pret_model->insert_row('purchase.t_retur_purchase_detail', $detail_ret)) {
                $sql = "UPDATE purchase.t_dtl_receive_order SET qty_retur = qty_retur + " . $obj->qty . " WHERE kd_produk = '" . $obj->kd_produk . "' AND no_do = '".$no_do."'";
		$this->pret_model->query_update($sql);
                
                $sql = "UPDATE purchase.t_purchase_detail SET qty_retur = qty_retur + " . $obj->qty . " WHERE kd_produk = '" . $obj->kd_produk . "' AND no_po = '".$obj->no_po."'";
		$this->pret_model->query_update($sql);
                if ($this->pret_model->insert_row('inv.t_trx_inventory', $trxinventory)) {
                    if (!$stokexists) {
                        if ($this->pret_model->insert_row('inv.t_brg_inventory', $brg_inventory)) {
        ...
        if ($header_result && $detail_result > 0) {
            $result = '{"success":true,"errMsg":"","printUrl":"' . site_url("pembelian_retur/print_form_ro/" . $no_retur) . '"}';
        } else {
            $result = '{"success":false,"errMsg":"Process Failed.."}';
        }
        echo $result;
    ...
    public function print_form_ro($no_retur = '') {
    ...
    public function get_rows_lokasi() {
        ...
        $result = $this->pret_model->get_rows_lokasi($kd_produk,$kd_peruntukan_supp,$kd_peruntukan_dist, $search, $start, $limit);
